              ...
              <TableRow key={row.id} className={`cursor-pointer border-slate-100 hover:bg-slate-50 ${index % 2 === 1 ? 'bg-slate-50/40' : ''}`} onClick={() => setSelectedRow(row)}>
                ...
                <TableCell className="whitespace-nowrap text-right">
                  {row.status === 'Open' && row.qty === row.remainingQty ? (
                    <>
                      {data?.capabilities.update ? <button className={`${rowActionButtonClass} mr-1.5`} type="button" onClick={(event) => { event.stopPropagation(); openEditForm(row) }}>แก้ไข</button> : null}
                      {data?.capabilities.cancel ? <button className={`${rowDangerActionButtonClass} mr-1.5`} type="button" onClick={(event) => { event.stopPropagation(); openCancelDialog(row) }}>ยกเลิก</button> : null}
                    </>
                  ) : null}
                  <button
                  ...
                  {data?.capabilities.shortClose && shouldShowShortCloseButton(row) ? (
                    <button
        ...
        <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-5 text-sm">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <UiButton className="bg-emerald-600 font-normal hover:bg-emerald-700 text-white rounded-md h-9 transition-colors" size="xs" type="button" variant="default" onClick={onAddItem}>+ เพิ่มรายการ</UiButton>
            </div>
