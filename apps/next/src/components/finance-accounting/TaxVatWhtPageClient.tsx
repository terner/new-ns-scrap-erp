...
function Donut({ input, net, output }: { input: number; net: number; output: number }) {
  ...
  return (
    <svg viewBox="0 0 200 200" className="h-44 w-full">
      ...
      <text fill={net > 0 ? '#dc2626' : net < 0 ? '#059669' : '#64748b'} fontSize="13" fontWeight="bold" textAnchor="middle" x="100" y="115">{money(net)}</text>
    </svg>
  )
}
...
function WhtBox({ label, tone, value }: { label: string; tone: 'amber' | 'purple' | 'red'; value: string }) {
  ...
  return (
    <div className={`rounded-md border p-3.5 transition-all ${color}`}>
...
function CalendarBars({ max, row }: { max: number; row: TaxPayload['taxCalendar'][number] }) {
  ...
  return (
    <div className="min-w-[96px] rounded-md border border-slate-100 bg-slate-50/50 p-2.5 text-center text-xs">
      <div className="flex h-32 items-end justify-center gap-1.5">
        ...
        <span className={`w-4 rounded-t-sm ${row.vatPayable > 0 ? 'bg-red-500' : row.vatPayable < 0 ? 'bg-emerald-700' : 'bg-slate-400'}`} style={{ height: height(row.vatPayable) }} />
      </div>
...
function StatCard({ label, sub, tone, value }: { label: string; sub?: string; tone: 'amber' | 'blue' | 'emerald' | 'purple' | 'red' | 'slate'; value: string }) {
  const toneStyles = {
    ...
    red: { border: 'border-red-200', bg: 'bg-red-50/30', text: 'text-red-700', iconBg: 'bg-red-100/60 text-red-600', emoji: '' },
    slate: { border: 'border-slate-200', bg: 'bg-slate-50/50', text: 'text-slate-700', iconBg: 'bg-slate-100 text-slate-600', emoji: '' }
  }[tone]
          ...
          <tbody className="divide-y divide-slate-100 text-slate-700">
            ...
            {!isLoading && sortedRows.map((item) => (
              <tr key={`${item.source}-${item.no}`} className="hover:bg-slate-50/50 transition-colors">
                ...
                {hasDoc ? <Td align="center">{item.hasDoc ? 'มี' : 'ขาด'}</Td> : null}
              </tr>
            ))}
          </tbody>
        </table>
