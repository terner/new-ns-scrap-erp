  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <select className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm md:w-72" value={accountId} onChange={(event) => setAccountId(event.target.value)}>
        ...
        <button className="rounded-lg bg-blue-600 px-3 py-2 text-sm text-white opacity-60" disabled type="button">Import CSV</button>
        <button className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white opacity-60" disabled type="button">Auto Match</button>
      </div>
...
function Stat({ label, tone, value }: { label: string; tone?: 'matched' | 'unmatched' | 'ignored' | 'erp'; value: number }) {
  ...
  return <div className={`rounded-xl p-3 shadow ${styles}`}><div className="text-xs opacity-80">{label}</div><div className="text-xl font-bold">{value}</div></div>
}
...
function Panel({ children, title, tone }: { children: React.ReactNode; title: string; tone: 'bank' | 'erp' }) {
  ...
  return <div className="overflow-hidden rounded-xl bg-white shadow"><h3 className={`border-b px-4 py-3 font-semibold ${heading}`}>{title}</h3><div className="max-h-96 overflow-x-auto">{children}</div></div>
}
