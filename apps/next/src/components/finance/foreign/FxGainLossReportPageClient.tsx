    ...
    <section className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200/60 bg-white p-4 shadow-sm">
        ...
        <div className="hidden lg:flex flex-wrap items-center gap-2">
          ...
          <Select aria-label="ประเภท" className="h-9 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" value={refType} onChange={(event) => setRefType(event.target.value)}>
          ...
          {showMobileFilters && (
            <div className="grid grid-cols-1 gap-2 pt-2 border-t border-slate-100 animate-in slide-in-from-top-2 duration-100">
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="text-xs text-slate-500">
                  สกุลเงิน
                  <Select aria-label="สกุลเงิน" className="mt-1 h-9 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" value={currency} onChange={(event) => setCurrency(event.target.value)}>
                    <option value="all">ทุกสกุล</option>
                    {(data?.filters.currencies ?? []).map((item) => <option key={item} value={item}>{item}</option>)}
                  </Select>
                </label>
                <label className="text-xs text-slate-500">
                  ประเภท
                  <Select aria-label="ประเภท" className="mt-1 h-9 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" value={refType} onChange={(event) => setRefType(event.target.value)}>
                    <option value="all">ทุกประเภท</option>
                    {(data?.filters.refTypes ?? []).map((item) => <option key={item} value={item}>{item}</option>)}
                  </Select>
                </label>
