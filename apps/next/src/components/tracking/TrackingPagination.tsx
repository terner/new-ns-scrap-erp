    ...
    <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-600">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select
          ...
          className="h-9 w-auto rounded-md border border-slate-300 bg-white px-2 py-1 text-sm outline-none"
          value={pageSize}
          onChange={(event) => {
            onPageSizeChange(Number(event.target.value) as TrackingPageSize)
            onPageChange(1)
          }}
        >
          {trackingPageSizeOptions.map((option) => (
