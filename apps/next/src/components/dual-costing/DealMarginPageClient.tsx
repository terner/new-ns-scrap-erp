      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-2xl bg-white p-4 shadow">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <StatusBox label="Fully" tone="emerald" value={data?.summary.fullyMatched ?? 0} />
            <StatusBox label="Partial" tone="amber" value={data?.summary.partial ?? 0} />
            <StatusBox label="None" tone="slate" value={data?.summary.none ?? 0} />
          </div>
        </div>
      </div>
...
function StatusBox({ label, tone, value }: { label: string; tone: 'amber' | 'emerald' | 'slate'; value: number }) {
  const classes = { amber: 'bg-amber-50 text-amber-700', emerald: 'bg-emerald-50 text-emerald-700', slate: 'bg-slate-50 text-slate-700' }[tone]
  return <div className={`rounded p-3 ${classes}`}><div className="text-2xl font-bold">{value}</div><div>{label}</div></div>
}
