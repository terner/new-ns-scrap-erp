...
export function ImpurityProductsPageClient() {
  ...
  function requestToggleActive(product: Product, active: boolean) {
    ...
    requestConfirmation({
      ...
      description: `ต้องการปิดการใช้งานสินค้าสิ่งเจือปน “${product.code} — ${product.name}” ใช่หรือไม่?`,
      destructive: true,
      onConfirm: () => handleToggleActive(product, active),
      title: 'ปิดการใช้งานสินค้าสิ่งเจือปน?',
    })
  }
...
function ProductForm({ isSaving, product, productTypes, productUnits, onCancel, onDirtyChange, onSubmit }: ProductFormProps) {
  ...
  function requestRemoveProductImage() {
    requestConfirmation({
      ...
      description: 'รูปสินค้าสิ่งเจือปนนี้จะถูกนำออกจากแบบฟอร์ม และจะมีผลเมื่อกดบันทึกสินค้า',
      destructive: true,
      onConfirm: removeProductImage,
      title: 'ลบรูปสินค้าสิ่งเจือปนนี้หรือไม่?',
    })
  }
      ...
      <div data-ns-dialog-header className="flex flex-col gap-3 bg-slate-900 dark:bg-[#0f172a] px-5 py-4 sm:flex-row sm:items-center sm:justify-between shrink-0">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
          <ActiveToggle checked={form.active} labelClassName="text-sm font-medium text-current" onChange={(active) => {
            ...
            requestConfirmation({ confirmLabel: 'ปิดการใช้งาน', description: 'ต้องการปิดการใช้งานสินค้าสิ่งเจือปนเมื่อบันทึกใช่หรือไม่?', destructive: true, onConfirm: () => update('active', false), title: 'ปิดการใช้งานสินค้าสิ่งเจือปน?' })
          }} />
