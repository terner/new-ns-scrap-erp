...
function salesLineRows(lines: SalesBillAnalyticsLine[] | undefined) { return (lines ?? []).map((line) => ({ amount: line.lineAmount, productId: line.productId == null ? '' : String(line.productId), qty: line.qty })) }
...
export async function buildDailyReportDashboard(filter: MainDashboardFilter): Promise<DailyReportPayload> {
  ...
  for (const bill of activeSales) { const key = bill.customers?.code ?? bill.customers?.name ?? ''; const current = topCustomers.get(key) ?? { amount: 0, bills: 0, gp: 0, id: bill.customers?.code ?? '', name: bill.customers?.name ?? '-', qty: 0 }; current.amount += toNumber(bill.total_amount); current.gp += toNumber(bill.gross_profit); current.qty += salesBillAnalyticsLineTotals(linesByBill.get(bill.id)).qty; current.bills += 1; topCustomers.set(key, current) }
  ...
  return {
    ...
    dailyReport: {
      analytics: { bySalesperson: Array.from(bySalesperson.values()).map((row) => ({ ...row, suppliers: row.suppliers.size })).sort((a, b) => b.amount - a.amount).slice(0, 10), dailyTrend: Array.from(dailyTrend.values()).sort((a, b) => a.label.localeCompare(b.label)).slice(-30), groupSummary: groupBreakdown.map((row) => ({ amount: row.buyAmt + row.sellAmt, group: row.group, qty: row.buyQty + row.sellQty })), rangeKpi: { cogs, expenseAmount, gp: grossProfit, gpPct: salesAmount > 0 ? grossProfit / salesAmount * 100 : 0, netProfit: salesAmount - cogs - expenseAmount, purchaseAmount, purchaseCount: activePurchases.length, purchaseQty: activePurchases.reduce((sum, row) => sum + purchaseBillItemQty(row), 0), salesAmount, salesCount: activeSales.length, salesQty: activeSales.reduce((sum, row) => sum + salesBillAnalyticsLineTotals(linesByBill.get(row.id)).qty, 0) }, topCustomers: Array.from(topCustomers.values()).map((row) => ({ ...row, gpPct: row.amount > 0 ? row.gp / row.amount * 100 : 0 })).sort((a, b) => b.amount - a.amount).slice(0, 10), topProductsIn: Array.from(productIn.values()).sort((a, b) => b.amount - a.amount).slice(0, 5), topProductsOut: Array.from(productOut.values()).sort((a, b) => b.amount - a.amount).slice(0, 5), topSuppliers: Array.from(topSuppliers.values()).sort((a, b) => b.amount - a.amount).slice(0, 10) },
