...
describe.runIf(enabled)('FCD receipt lifecycle reconciliation integration', () => {
  beforeAll(async () => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await postFcdReceiptAccountSplits(tx, { actor, bankStatementDocNos: [`BSL${suffix}`], branchId: branch.id, carryingThbAmount: 3500, currencyCode: foreignCurrencyCode, date: '2026-07-01', rate: 35, receiptDocNo, receiptId: receipt.id, sourceEventKey: `${actor}:receipt`, splits: [{ accountCode: fcdAccount.code, nativeAmount: 100 }] })
