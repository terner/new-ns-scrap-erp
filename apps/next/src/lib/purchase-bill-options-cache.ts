...
export async function cachedPurchaseBillOptions<T>(url: string) {
...
export function invalidatePurchaseBillOptionsCache() {
  for (const key of cache.keys()) {
    if (key.endsWith('/api/purchase/bills/options') || key.includes('/api/purchase/bills/options?')) cache.delete(key)
  }
}
