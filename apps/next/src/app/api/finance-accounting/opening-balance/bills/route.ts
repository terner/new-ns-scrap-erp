...
async function resolve(rows: Row[], branchCode: string, importType: 'purchase' | 'sales'): Promise<ResolvedRow[]> {
  ...
  return rows.map((row) => { const product = productsByKey.get(`${norm(row.productName)}|${norm(row.productType)}`) ?? products.find((candidate) => norm(candidate.name) === norm(row.productName)); const party = partiesByName.get(norm(row.partyName)); const error = !branch ? `ไม่พบสาขา ${branchCode}` : existing.has(row.docNo) ? `เลขที่เอกสาร ${row.docNo} มีอยู่แล้ว` : !party ? `ไม่พบ${importType === 'purchase' ? 'ผู้ขาย' : 'ผู้ซื้อ'} ${row.partyName}` : !product ? `ไม่พบสินค้า ${row.productName}` : undefined; return { ...row, error, partyId: party?.id, productCode: product?.code ?? '', productId: product?.id, status: error ? 'error' : 'ready', warning: Math.abs(row.lineAmount - row.quantity * row.unitPrice) > 0.05 ? 'ยอดรวมบรรทัดไม่เท่ากับ จำนวน × ราคา' : undefined } })
}
