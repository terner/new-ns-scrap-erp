...
import { readProfitCostSummary } from '@/lib/server/profit-cost-report-reader'

export const runtime = 'nodejs'
export async function GET(request: NextRequest) {
  try {
    const { context, startedAt } = await authorizeProfitCostRequest(request)
    return profitCostJson(await readProfitCostSummary(await scopedProfitCostFilter(request, context)), startedAt)
