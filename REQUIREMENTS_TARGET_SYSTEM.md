...
- Status convert ไม่ใช่ production order และไม่ควรถูกรวมเป็น `PO2` output; ถ้ารายงาน Production ต้องแสดงผลจาก `SC` ให้แยก source label จาก production output
...
- Production Report / Yield ที่ต้องแสดง RM/FG status conversion ต้องอ่าน `SC` เป็น source แยกจาก `PO2` เพื่อไม่ให้ yield/loss ของใบสั่งผลิตเพี้ยน
