...
- Grade Adjustment
- Status Convert
- WTO pending_out / Sales Bill stock-out
...
Customer/Supplier branch eligibility เป็นส่วนหนึ่งของ master data:

- Customer และ Supplier ต้องผูกกับสาขาที่ใช้งานได้ผ่าน mapping table แยก ไม่เก็บเป็นสาขาเดียวบน master row เพราะคู่ค้า 1 รายอาจใช้ได้หลายสาขา
- mapping ต้องมีสถานะ active/inactive และระบุ `primary branch` ได้ เพื่อให้ master page แสดงว่าสาขาไหนเป็นสาขาหลักของคู่ค้านั้น
- transaction page ที่มีสาขาและ Customer/Supplier ต้อง filter option จาก mapping active ของสาขานั้นเท่านั้น และ API ต้อง validate ซ้ำใน transaction เดียวกันก่อนบันทึก
- ถ้า Customer/Supplier ไม่มี active mapping กับสาขาที่เลือก ต้องห้ามเลือกในเอกสารใหม่และต้อง reject payload; ห้าม fallback เป็นทุกสาขาหรือเดาจากชื่อสาขา/code
- เอกสารเก่าต้องยังแสดง snapshot คู่ค้าเดิมได้ แต่การสร้าง/แก้เอกสารใหม่ต้องใช้ mapping ปัจจุบันเป็น source of truth
...
- ผู้ขายใน PO Buy, WTI, Purchase Bill, ADV/PMA/PMT/AP ที่ผูกสาขาต้องเลือกได้เฉพาะ Supplier ที่มี active `supplier_branches` ของสาขานั้น และ backend ต้อง reject ถ้า Supplier ไม่ถูกผูกกับสาขาเอกสาร
...
- บิลรับซื้อ Trading ต้องรองรับหลายรายการสินค้าในใบเดียว โดยแต่ละรายการเลือกสินค้า, PO Buy อ้างอิงถ้ามี, Gross, หัก, น้ำหนักสุทธิ, ราคา/กก., จำนวนตัดบิล, ราคาหน้าใบสำหรับ commission, และยอดรวม; น้ำหนักสุทธิ = Gross - หัก และยอดรวม = น้ำหนักสุทธิ x ราคา/กก.
- เมื่อเลือก PO Buy ในบิลรับซื้อ Trading แล้ว ถ้าสินค้าต้องการรับจริงไม่ครบตาม PO Buy ผู้ใช้ต้องสามารถเพิ่มรายการเองด้านล่างเพื่อเพิ่มสินค้า/น้ำหนักให้ครบตาม PO Buy หรือแยกส่วนที่เกิน/ไม่อ้าง PO เป็น Spot Buy ตาม rule แหล่งซื้อ
...
- เมื่อบันทึกบิลรับซื้อใหม่สำเร็จ ระบบต้องส่ง LINE Flex Message ไปยังกลุ่มที่ผูกกฎประเภทเอกสาร `PB` โดยความล้มเหลวของ LINE ต้องไม่ rollback บิล และหากไม่มีกฎตรงต้องไม่ fallback ไปกลุ่มอื่น

### 5.4 Sales Management

- สร้างและแก้ไขบิลขาย
- เก็บ line items พร้อม weight และราคา
- ลูกค้าใน PO Sell, WTO, Sales Bill, Receipt/AR ที่ผูกสาขาต้องเลือกได้เฉพาะ Customer ที่มี active `customer_branches` ของสาขานั้น และ backend ต้อง reject ถ้า Customer ไม่ถูกผูกกับสาขาเอกสาร
...
- บิลขาย Trading สามารถเลือกผูกหรือไม่ผูกกับ PO Sell ได้; ถ้าผูก PO Sell ต้องตัดยอด commitment/remaining ของ PO Sell ระดับรายการ แต่ไม่ตัด stock ledger
- สินค้าที่เปิดบิลขายประเภท Trading ไม่ต้องตัดออกจาก Stock ไม่ว่าจะเป็น Trading ที่เปิด PO Sell หรือไม่เปิด PO Sell
- บิลขาย Trading จะถูกนำไป match กับบิลรับซื้อ Trading ที่ผูกหรือไม่ผูก PO Buy ก็ได้ และต้องส่งต่อไป Trading Matching เพื่อคำนวณ allocation/margin
- ทองเหลือง/ทองแดงที่เป็น Trading ไม่ต้องตัด Stock ตัวเองเมื่อเปิดบิลขาย ให้เข้า Trading Matching เพื่อจับคู่ขาย/ตัดขายนอกระบบ
- ถ้ามีสินค้าบริษัทจาก Stock ที่ต้องตัด stock จริง ต้องมาจาก `WTO` line ที่สร้าง `pending_out` ไว้แล้ว และ `Sales Bill` เป็น owner ของ stock ledger stock-out
- บิลขายต้องตัด PO Sale/PO Sell ได้ระดับรายการสินค้า ทั้งรายการที่มาจาก WTO flow และรายการ Trading ที่ผูก PO Sell โดย stock movement เกิดเฉพาะ flow ที่เป็น Stock เท่านั้น
...
- เมื่อบันทึกบิลขายใหม่สำเร็จ ระบบต้องส่ง LINE Flex Message ไปยังกลุ่มที่ผูกกฎประเภทเอกสาร `SB` โดยความล้มเหลวของ LINE ต้องไม่ rollback บิล และหากไม่มีกฎตรงต้องไม่ fallback ไปกลุ่มอื่น
...
- Customer return
- Grade adjustment
- Status convert (RM / FG)
- รองรับ lot และ warehouse movement
- Stock transfer ใช้โอนสินค้าเท่านั้น ไม่กระทบ revenue/AR/AP; เมื่อส่งแล้วต้องเขียน paired `ST` stock ledger เป็น stock-out ต้นทางและ stock-in ปลายทาง พร้อม source available และ source unit cost/kg ก่อนบันทึก
- หน้า Stock transfer ต้อง filter ด้วยเลขที่/วันที่/น้ำหนักรวม, แสดงแก้ไขล่าสุดโดย/เวลาและมูลค่ารวม, และ modal เป้าหมายไม่ใช้ผู้ส่ง/ผู้รับ/Lot manual
- Stock transfer แก้ไข/ยกเลิกได้เฉพาะรายการที่ยังไม่ส่งเข้าตัด stock; หลังส่งแล้วห้ามแก้ไขข้อมูลเดิม และถ้าต้องยกเลิกในอนาคตต้องใช้ reversal/dependency policy แยก
- Stock Count Adjust ใช้ตรวจนับ stock จริงและปรับยอดให้ตรงระบบสำหรับ stock หาย, stock เกิน, audit และ cycle count; ระบบต้องสร้าง `ADJ` adjustment ledger และ stock correction record ที่ trace ได้
- Stock Count Adjust ต้องรองรับสินค้าทุกประเภท/ทุกสถานะที่ระบบมี stock bucket ให้ปรับได้, โดยแก้ไขรายการนับย้อนหลังได้ไม่เกิน 7 วัน และการแก้ไขต้องเก็บ `updated_by/updated_at` หรือ correction/reversal trail ห้ามลบประวัติ
- Stock Count Adjust ต้องใช้เหตุผลเป็น fixed options ในหน้า ไม่ต้องมี master แยก: หาของไม่เจอ, นับจริง 0 แต่ระบบมี, นับได้เกินระบบ, สูญหาย, เสียหาย, ผิดสาขา/คลัง, อื่นๆ
- Stock Count Adjust ต้องแสดงราคาต่อกก. และมูลค่ารวมเป็นจำนวน signed: `มูลค่า Loss = unit price/kg ของวันนับ * diff ที่เป็น loss` เป็นค่าติดลบ และ `มูลค่าเกิน = unit price/kg ของวันนับ * diff ที่นับเกิน` เป็นค่าบวก; wording บนหน้าต้องใช้ `มูลค่ารวม (บาท)` ไม่ใช่ `มูลค่า Note`
- Stock Count Adjust target ต้องระบุชัดว่าการปรับยอดและมูลค่า correction อาจกระทบ WAC และ margin ได้; ถ้ายังใช้ note-only runtime ต้องถือเป็น implementation gap ไม่ใช่ target accounting policy
- Stock status convert ใช้แปลง bucket `RM <-> FG` ของสินค้าเดิม/สาขาเดิม/คลังเดิม/lot เดิม โดยเขียน paired stock ledger `SC`, ลด stock ต้นทาง, เพิ่ม stock ปลายทาง, ใช้ source WAC ต่อให้ target bucket, และห้ามทำให้ source stock ติดลบ
- Status convert ไม่ใช่ production order และไม่ควรถูกรวมเป็น production output / `PO2` output; ถ้ารายงาน Production ต้องแสดงผลจาก `SC` ให้แยก source label จาก production output
- Grade adjustment / stock convert ต้องใช้ Cost Pool เป็น source ต้นทุน: เลือก source lots ด้วย FIFO/LIFO/Highest/Lowest/Manual, consume source Cost Pool availability, สร้าง target Cost Pool row เป็น `Regrade` / `Conversion`, และบันทึก Allocation Ledger / Match Logs เพื่อ trace source lot -> target lot
- Grade adjustment ต้องบังคับ target qty `<=` source qty; ถ้าน้อยกว่าต้องเก็บ loss/yield tracking พร้อมเหตุผล และห้าม fallback/hard-code cost เมื่อ Cost Pool ไม่พอ
- Grade adjustment reverse ต้องเป็น append-only: คืน source Cost Pool, reverse target regrade Cost Pool effect, mark allocation/match logs reversed, และ block ถ้า target stock ถูกใช้ต่อจน reverse แล้วติดลบ
...
- WIP คงเหลือเป็น metric ภายใน production order/report/reconciliation; standalone WIP report ไม่อยู่ใน active menu target
- Production Report / Yield ต้องวิเคราะห์ผลผลิตจริงเทียบกับวัตถุดิบที่ใช้ด้วยสูตร `Yield % = Output Qty / Input Qty * 100`
- Production Report / Yield ต้องแสดง `Loss Value (บาท)` ด้วยสูตร `Loss Qty * (RM Cost / Input Qty)` และต้องแยกชื่อ `RM บาท/กก.` ออกจาก `ต้นทุนผลิต บาท/กก. = Total Production Cost / Output Qty`
- Production Report / Yield ต้องใช้ PI/PO2 stock ledger/read-model facts เป็น source ของ Input, Output, WIP, Loss, RM Cost, Process Cost และห้าม fallback ไป stale order totals
- Production Report / Yield ที่ต้องแสดง RM/FG status conversion ต้องอ่าน `SC` เป็น source แยกจาก `PI/PO2` เพื่อไม่ให้ yield/loss ของใบสั่งผลิตเพี้ยน
- Production Report / Yield ต้องมี block `WIP คงเหลือ`, `ผลผลิตแยกตามสินค้า`, ตารางรายละเอียดใบสั่งผลิต และ CSV export จากข้อมูลตารางตาม filter ที่เห็นบนหน้า
- Yield / Loss report
...
- PO Buy
- Trading purchase bill ที่ไม่เข้า stock และเป็น cost source สำหรับ Trading Sales Bill
- Trading sales bill ต้องเลือกบิลรับซื้อ/cost source ได้หลายใบ, เติมรายการขายจากบิลรับซื้อหรือรายการ Trading ที่จะส่งไปจับคู่, allocate ไป PO Sell ได้ระดับรายการถ้ามี, และต้องบันทึก allocation กลับไปยัง source เพื่อกันขายซ้ำและคำนวณ margin โดยไม่เพิ่ม stock-out line ในบิล Trading
- Trading Matching เป็น read-only allocation/margin surface หลังจาก Trading Sales Bill ถูกจับคู่ต้นทุนแล้ว ไม่ใช่หน้าสร้าง match ใหม่
- Trading Matching ต้องใช้ Sales Bill เป็น business reference หลัก แสดง Cost, Sales Amount, Expected GP, และ `GP% = GP / Sales Amount`; ไม่แสดง Deal No เป็นคอลัมน์หลัก, status donut/filter/column, duplicate cleanup, cloud pull, recalc cost, new match, reverse, หรือ unmatched ฝั่งขาย
- การแก้/Reverse Trading allocation ต้องทำผ่าน source Purchase Bill / Sales Bill correction flow และ audit ledger ไม่ใช่จากหน้า Trading Matching
...
- Trading dashboard สำหรับ trader/operator monitor ภาพรวม Trading deal ทั้งระบบ ไม่ใช่ accounting dashboard
- Trading dashboard ต้องรวมข้อมูลจาก PO Buy, PO Sell, Cost Pool, Allocation/Trading deal fact, Sales Bill และ Stock/availability read model เพื่อดู margin, allocation, pending และ exposure
- Trading dashboard ต้องตอบคำถามหลักได้: deal/product/supplier/customer ไหนกำไร, allocation ค้างไหม, exposure/commitment เท่าไหร่, stock หรือ cost source พอขายไหม, pending sell/buy ค้างเท่าไหร่
- Trading dashboard ต้องมีมุมมองหลักอย่างน้อย `Trading by Product`, `Trading Purchase`, และ `Trading Sales`
- ทุกมุมมองใน Trading dashboard ต้อง filter ได้ด้วย date from/date to, supplier, customer, และเลขที่บิล; `Trading by Product` ต้องมี product filter เพิ่ม
- Trading dashboard ใช้ยอดซื้อ/ขายก่อน VAT เป็นหลัก; `Matched COGS` คือ cost ที่ match/allocate มาจาก Trading cost source หรือ cost allocation fact ไม่ใช่ยอด AP/AR accounting
- Trading dashboard formula: `Trading GP = Sales - Matched COGS`, `GP% = GP / Sales`; product GP รวมตามสินค้าโดยใช้ Qty/Sales จาก Sales Bill lines และ Matched COGS ที่ allocate มายัง line/product นั้น
- Trading dashboard drilldown ต้องไป `Trading Matching`, `Deal Margin`, `Allocation/Cost Allocation Ledger`, source PB/SB/PO เมื่อ route พร้อม
- Trading dashboard ห้ามใช้เป็นหน้า cash/bank/payment/AP/AR settlement หรือ financial statement dashboard
- Trading allocation and margin tracking
- PO outstanding
