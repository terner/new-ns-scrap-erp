...
- `/api/analytics-dashboard` ใช้ line reader แบบ lightweight ที่ select เฉพาะ `sales_bill_id`, `product_id`, `product_name_snapshot`, `qty` และ `line_amount`; ห้ามเรียก `salesBillLineFactsByBillId` เพราะ analytics ใช้ COGS จาก bill header และไม่ต้องใช้ allocation/trading/stock-ledger cost detail
