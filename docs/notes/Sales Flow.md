...
- บิลขาย Trading ต้องเริ่มจากการเลือกบิลรับซื้อก่อน และเลือกได้หลายบิลรับซื้อในบิลขายเดียว
- เมื่อเลือกบิลรับซื้อแล้ว ระบบต้องแสดงรายการสินค้าที่จะขายจากบิลรับซื้อนั้น แต่ผู้ใช้ยังเพิ่มรายการเองได้
- รายการที่เพิ่มเองจาก stock ต้องตัด stock ตามจำนวน/น้ำหนักที่ขาย ส่วนรายการที่มาจากบิลรับซื้อ Trading ไม่ตัด stock ซ้ำ
- บิลขาย Trading ต้องรองรับการตัด PO Sell ได้ในระดับรายการสินค้า
...
| บิลขาย | เปิดบิล / ตั้งลูกหนี้ / ยืนยันยอดขาย | Pending Sale อ้างอิง, บิลซื้อ Trading อ้างอิง, หรือขายตรง, Customer, ราคา, VAT, เครดิตเทอม, หมายเหตุ | `SB{branchCode}{YYMM}-NNNN` | `เปิดอยู่` |
...
|---|---|---|---|---|
| `UC-SAL-01` | ขายตาม PO Sell แล้วเบิกออกรอบิลก่อน | ใช่ | `Flow ขายแบบมี PO Sell` | ครบระดับ business flow |
| `UC-SAL-02` | ขาย Trading โดยอ้างบิลรับซื้อหลายใบ | ใช่ | `Flow บิลขาย Trading` | ครบระดับ business flow |
...
| `UC-SAL-08` | บิลขาย Trading มีทั้ง line จากบิลซื้อและจาก stock | ใช่ | `Flow บิลขาย Trading` | ครบระดับ business rule |
...
|---|---|---|
| `UC-SAL-F01` | allocation ระดับ line ระหว่าง WTO กับ Sales Bill เมื่อ 1 WTO แตกไปหลายบิลขาย | business rule เริ่มชัดแล้ว แต่ UI allocation ยังต้อง refine |
...
| 3 | เปิดบิลขายโดยอ้าง WTO | เลือก Customer, สาขา, WTO ที่เกี่ยวข้อง, ตรวจรายการสินค้า/น้ำหนัก, กรอกราคา/VAT/เครดิตเทอม | `SB...` | Sales Bill = `เปิดอยู่` | ตั้งลูกหนี้/AR และบันทึก usage ของ WTO ไปยังบิลขาย |
| 4 | ระบบอัปเดต WTO | ไม่ต้องกรอกเพิ่ม | ไม่มี | WTO = `ออกบิลแล้ว` | กันการแก้ไข/ยกเลิก WTO หลังถูกใช้ไปออกบิลแล้ว |
...
- `WTO` ที่ถูกใช้แล้วต้องแสดงสถานะ `ออกบิลแล้ว`
...
| 3 | ตรวจ/แก้รายการขาย | ปรับจำนวน/น้ำหนักที่จะขาย ราคา และเลือก PO Sell ที่ต้องตัดถ้ามี | ไม่มี | Draft | รายการจากบิลรับซื้อจะถูกใช้สำหรับ Trading matching/COGS ไม่ตัด stock |
| 4 | เพิ่มรายการเองจาก stock ถ้าต้องขายเพิ่ม | เลือกสินค้า คลัง จำนวน/น้ำหนัก ราคา และ PO Sell ถ้ามี | ไม่มี | Draft | รายการที่เพิ่มจาก stock ต้องตรวจ stock พร้อมขายและตัด stock เมื่อบันทึก |
| 5 | บันทึกบิลขาย | ตรวจยอดรวม VAT ส่วนลด เครดิตเทอม และหมายเหตุ | `SB...` | Sales Bill = `เปิดอยู่` | ตั้งลูกหนี้/AR, บันทึก allocation ไปบิลรับซื้อ, ตัด stock เฉพาะ stock line, และตัด PO Sell ตาม line allocation |
...
- บิลขาย Trading ต้องเลือกบิลรับซื้อก่อนรายการสินค้า เพื่อให้ระบบรู้ต้นทุน/ยอดคงเหลือที่ขายได้
- เลือกบิลรับซื้อได้หลายใบ และหนึ่งบิลรับซื้อสามารถถูกนำไปขายหลายบิลขายได้จนยอดคงเหลือหมด
- รายการสินค้าในบิลขายต้องมี source ต่อ line เช่น `PURCHASE_BILL`, `STOCK`, หรือ source equivalent
- รายการ source `PURCHASE_BILL` ใช้ยอดจากบิลรับซื้อ Trading และใช้ใน Trading Matching / Deal Margin
- รายการ source `STOCK` คือรายการที่ผู้ใช้เพิ่มเองและต้องตัด stock เหมือนขายตรง
- การตัด PO Sell ต้องทำระดับ line item เพราะหนึ่งบิลขายอาจมีทั้ง line จากบิลรับซื้อและ line จาก stock
- ถ้าเลือก PO Sell แล้ว จำนวน/น้ำหนักที่ตัดต้องไม่เกินยอด PO Sell ที่ยังไม่ออกบิล/ยังไม่ถูกตัด
- ถ้าบิลขายมีทั้ง line จากบิลรับซื้อ Trading และ line จาก stock ให้ถือเป็น mixed-source sales bill แต่ผลต่อ stock ต้องเกิดเฉพาะ stock line เท่านั้น
...
หมายเหตุ: ถ้าต้องบันทึกเอกสารส่งของ/น้ำหนักขาออก ให้สร้าง `WTO ใบส่งของ` ที่ `รายการประจำวัน > ชั่งสินค้า / รับ-ส่งของ` และติดตาม/ค้นหาได้ที่ `รายการประจำวัน > รายการใบรับ-ส่งของ`
...
| 5 | ระบบบันทึกบิลขาย, allocation, stock line, และ PO Sell cut | ระบบอัตโนมัติ | - |
...
| เปิดบิลขาย Trading | ต้องขายจากบิลรับซื้อ Trading ก่อน และอาจมี stock line เพิ่มเอง | ตัด stock เฉพาะ line ที่ source เป็น stock |
...
| คลัง | เฉพาะขายตรง stock หรือ stock line ใน Trading sale | ดึงจาก PSALE หรือกรอก | ถ้ามาจาก PSALE หรือบิลรับซื้อ Trading ไม่ควรตัดซ้ำ |
...
| แหล่งรายการสินค้า | ใช่ | ระบบ derive หรือผู้ใช้เลือก | `PSALE`, `PURCHASE_BILL`, `STOCK`, หรือ source equivalent เพื่อคุมผล stock/cost |
...
- ต้องเลือกบิลรับซื้ออย่างน้อย 1 ใบก่อนสร้างรายการตั้งต้น
- เลือกบิลรับซื้อได้หลายใบ แต่ต้องเป็นบิลที่ยังมียอดคงเหลือให้ขาย/จับคู่
- จำนวน/น้ำหนักของ line ที่ source เป็น `PURCHASE_BILL` ต้องไม่เกินยอดคงเหลือของบิลรับซื้อนั้น
- Line ที่ source เป็น `PURCHASE_BILL` ต้องบันทึก allocation กลับไปยังบิลรับซื้อเพื่อใช้ Trading Matching, Deal Margin และกันขายซ้ำ
- Line ที่ source เป็น `STOCK` ต้องตรวจ stock พร้อมขายและสร้าง stock ledger ใน transaction เดียวกับ sales bill
- ถ้า line ใดตัด PO Sell ต้องตรวจว่าไม่เกินยอด PO Sell ที่ยังตัดได้ และบันทึก allocation กลับ PO Sell ระดับ line
- Mixed-source sales bill ทำได้ แต่ต้องคำนวณ COGS/GP แยกตาม source line ก่อนรวมยอดบิล
...
| ยอดขายจากบิลรับซื้อ Trading | `sales_bill_lines` + `sales_bill_purchase_allocations` หรือ allocation table equivalent |
| ยอดขายจาก stock ในบิล Trading | `sales_bill_lines` + `stock_ledger` / `inventory_transactions` เฉพาะ stock line |
...
- ปรับ `/sales/bills` สำหรับ Trading sale ให้เลือกบิลรับซื้อได้หลายใบ เติมรายการสินค้าอัตโนมัติ เพิ่ม stock line เองได้ และ allocate line กลับบิลรับซื้อ/stock/PO Sell
