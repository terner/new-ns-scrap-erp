...
- WTO detail/modal should show cost snapshot on the product breakdown table for users allowed to see stock/sales cost:
  ...
  - line/portion row: cost snapshot used by that portion, or `รอยืนยันราคาเฉลี่ย` when draft pending_out has no snapshot yet.
...
- WTO เป็น `pending_out` source โดยตรง: เมื่อบันทึก WTO draft ต้องกัน stock เป็น `pending_out` และแสดงใน Stock Balance เป็น `รอออก`; ราคาเฉลี่ยจะถูก snapshot เฉพาะตอนยืนยันครั้งแรกหรือส่วนเพิ่ม/SKU ใหม่หลังยืนยันเท่านั้น
...
- WTO draft save สร้าง `pending_out` เพื่อกัน stock แต่ยังไม่ snapshot ราคาเฉลี่ย; WTO confirm จึง snapshot ราคาเฉลี่ยของ pending_out ที่ยืนยันแล้ว
