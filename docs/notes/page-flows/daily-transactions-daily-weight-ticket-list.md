...
- ตาราง WTI/WTO แสดง snapshot ร่างล่าสุดของผู้ดูและผู้ใช้อื่นในสาขาที่ผู้ดูมีสิทธิ์เป็นแถวไฮไลต์ `กำลังกรอก` ในตารางเดิม ไม่สร้างแท็บแยก พร้อมคำบรรยาย `สิ่งที่เพิ่งบันทึกร่าง` ที่ server สรุปจาก diff ระหว่าง snapshot ก่อน/หลัง; กดแถวหรือการ์ดเพื่อเปิด modal อ่านอย่างเดียวจาก snapshot ชุดเดียวกับตาราง โดยไม่สร้าง request/polling เพิ่ม; ร่างใหม่ที่ยังไม่เลือกสาขาแสดงแก่ผู้มีสิทธิ์ดูเป็น `ยังไม่เลือกสาขา` เพื่อให้ผู้ใช้อื่นเห็นว่าเริ่มทำงานแล้ว แต่ยังไม่แสดงข้อมูลที่ต้องอาศัย branch scope จนกว่าจะมี trusted branch; ชื่อคู่ค้า/สินค้า/คลังที่แสดงต้อง resolve จาก active master ฝั่ง server ตาม trusted branch scope (หากหาไม่พบหรืออยู่นอก scope ให้ละเว้น) และห้ามแสดง `vehicleNo`, ชื่อจาก payload client หรือข้อความหมายเหตุ
...
| 2.1 | กรอกหรือเลือกข้อมูล, ยืนยัน/เปลี่ยนสินค้า, เพิ่มสินค้า หรือกดเพิ่มเต๋า | action ยืนยัน/เปลี่ยนสินค้าและเพิ่มสินค้า/เต๋า queue working draft ทันทีแบบ asynchronous; field ที่กรอกเอง queue หลังหยุดกรอกประมาณ 800ms โดยฟอร์มยังพิมพ์ต่อได้ |
| 2.2 | เปิดรายการ WTI/WTO เดิม | poll snapshot ล่าสุดทุก 3 วินาทีเฉพาะขณะหน้า visible แล้วแสดงเป็นแถว `กำลังกรอก` ในตาราง/การ์ดรายการเดิม พร้อม `activityDescription` ที่ server derive จาก snapshot diff (เช่น เพิ่มเต๋า — ทองแดง, เปลี่ยนคลัง — ทองแดง → RM-A, แก้น้ำหนักชั่ง — ทองแดง · 125.50 กก.); กดแถวเพื่อเปิด modal อ่านอย่างเดียวที่แสดงประเภท ผู้กรอก สาขา คู่ค้า สินค้า/จำนวนรายการ น้ำหนักรวม/หัก/สุทธิ และอัปเดตล่าสุดจาก state ชุดเดียวกับตาราง; ชื่อสินค้า/คลังในคำบรรยาย resolve จาก active master ฝั่ง server และไม่แสดง `vehicleNo`, ชื่อจาก payload client หรือข้อความหมายเหตุ พร้อมข้อมูลแบบอ่านอย่างเดียวของผู้ดูและผู้ใช้อื่นในสาขาที่มีสิทธิ์ |
...
- `GET/PUT/DELETE /api/daily/weight-ticket-form-drafts`
  ...
  - ทุก PUT ให้ server เปรียบเทียบ snapshot เดิมและ snapshot ใหม่เพื่อคำนวณ `lastChange` และ `activityDescription`; team feed จึงไม่เชื่อ action/context/description หรือ display label ที่ client ส่งมา. ชื่อคู่ค้า/สินค้า/คลังต้อง resolve จาก active master ฝั่ง server ตาม trusted branch scope (party/warehouse ต้องอยู่ใน branch scope และหาก lookup ไม่ได้ให้ละเว้น ไม่ fallback เป็นข้อความใน snapshot); เปิดเผยได้เฉพาะชนิด action, label ที่ resolve ได้ และน้ำหนักรวมปัจจุบัน ห้ามเผย `vehicleNo`, ชื่อที่ client กรอก, ข้อความ free-text เช่น note/remark, ชื่อไฟล์หรือ URL, ID/scope key, หรือค่าเดิมก่อนแก้ไข
...
- working draft เก็บเพียง snapshot ของผู้ร่างแบบ queue; action ยืนยัน/เปลี่ยนสินค้าและเพิ่มสินค้า/เต๋า queue ทันทีแบบ asynchronous ส่วน field ที่แก้เอง queue หลังหยุดกรอกประมาณ 800ms โดยไม่ block การพิมพ์. ผู้ร่างเท่านั้นที่อ่าน/เขียน/ลบร่างของตนเองได้; ผู้ที่มี `daily.weight_tickets.view` เห็น projection แบบอ่านอย่างเดียวของร่างผู้ดูและผู้ใช้อื่นที่อัปเดตใน 5 นาทีล่าสุดและอยู่ใน branch scope เดียวกัน. ร่างใหม่ที่ `visibility_branch_id` ยังว่างแสดงแก่ผู้มีสิทธิ์ดูเป็น `ยังไม่เลือกสาขา` เพื่อให้ผู้ใช้อื่นเห็นว่าเริ่มทำงานแล้ว โดยละเว้นคู่ค้า/คลังและข้อมูล branch-scoped จนกว่าจะมี trusted branch. Projection ส่งเฉพาะชื่อผู้ร่างจาก user record, ประเภท, สาขาที่เชื่อถือได้เมื่อมี, คู่ค้า/สินค้าและจำนวนรายการที่ resolve จาก active master ฝั่ง server, น้ำหนัก, เวลาอัปเดต และ `activityDescription` ที่ server derive จาก snapshot diff; modal อ่านอย่างเดียวใช้ projection/state ชุดนี้และรอบ poll เดิม จึงไม่เรียก endpoint รายละเอียดหรือสร้าง interval เพิ่ม. ชื่อคลังจะแสดงได้เฉพาะเมื่อ resolve จาก active master ใน branch scope เดียวกัน. ห้ามส่ง `vehicleNo`, display label หรือชื่อที่ client กรอก, ข้อความ free-text เช่น note/remark, ชื่อไฟล์หรือ URL, ID/scope key, ค่าเดิมก่อนแก้ไข หรือ action/context/description จาก client; reference ที่หาไม่พบหรืออยู่นอก scope ต้องละเว้น ไม่ fallback เป็นข้อความใน snapshot. คำบรรยายอาจระบุเฉพาะชนิด action, label ที่ resolve ได้ และน้ำหนักรวมปัจจุบัน. ทั้ง draft writer และ shared reader เป็น L5, `private, no-store`, ไม่มี browser/Redis persistence. ไม่มี document number, stock, pending-out, ledger, timeline, audit, PDF หรือ LINE side effect และถูกลบหลัง save จริงหรือกด `ยกเลิก`/`ยกเลิกและกลับรายการ` เฉพาะ revision ที่กำลังถืออยู่ เพื่อไม่ลบร่างใหม่จากอีกหน้าต่าง; การปิดด้วย Escape/backdrop หรือเปลี่ยนหน้าโดยไม่กด action ดังกล่าวคงร่างไว้สำหรับ recovery; save เอกสารจริงกับ cleanup ต้องเป็น transaction เดียวกัน และ revision conflict ต้อง rollback เอกสารจริง
...
- ทุก field ที่ผู้ใช้กรอกหรือเลือกเอง ให้บันทึกร่างอัตโนมัติแบบไม่ block การพิมพ์หลังหยุดกรอกประมาณ 800ms; ส่วนยืนยันสินค้าในรายการหรือกดเพิ่มรายการ/เพิ่มเต๋าให้ queue ทันที พร้อมสถานะสั้นและปุ่มลองใหม่เมื่อ write ไม่สำเร็จ. ถ้า baseline draft โหลดไม่สำเร็จให้คีย์ต่อได้ และการพิมพ์ครั้งถัดไปต้อง retry baseline ก่อนส่ง write ครั้งแรก
- แถว `กำลังกรอก` เป็นข้อมูล snapshot สำหรับติดตามงานในตารางเดิม ไม่ใช่สถานะธุรกิจ `draft` ของเอกสาร; กดเพื่อเปิด modal อ่านอย่างเดียวได้ แต่ไม่มีปุ่ม Save/Edit/Confirm/Print/Share และระบุ `activityDescription`/เวลา เพื่อไม่อ้างว่าเป็น collaborative editing หรือ presence ระดับวินาที
- สิทธิ์ draft แยกตามชนิดงานจริง: `new:*` ต้องมี `daily.weight_tickets.create`, `ticket:*` ต้องมี `daily.weight_tickets.update`, และทุกกรณียังต้องมี `daily.weight_tickets.view`; จึงไม่ให้ activity feed สะท้อนการสร้างหรือแก้ไขที่บทบาทนั้นไม่มีสิทธิ์ทำ
...
## Working Draft Visibility Checkpoint (2026-07-21)

- WTI/WTO working drafts are shown as highlighted `กำลังกรอก` rows inside their existing list table; there is no separate latest-updates tab. Clicking a row/card opens a read-only detail modal backed by the same polled projection state, with no second polling loop or detail request. The modal uses the shared neutral header, grouped cards, two-column mobile read-only layout, and sticky outline close action.
- The list polls the team draft summary every 3 seconds, so a colleague can see a new or changed draft in the same table without refreshing the page.
- Working draft rows reuse the list search, branch, date, type, and draft-status filters. They are display-only until the underlying document is saved/confirmed.
- The team summary intentionally exposes master-derived labels and totals only; typed vehicle numbers, remarks, attachments, and raw draft payload are not returned to other users.
- Each team-summary row includes only a keyed-HMAC selection token derived server-side from owner+scope, so poll refreshes cannot switch the open modal between same-name users while owner ids and scope keys remain unrecoverable and undisclosed. The reader is bounded to 100 candidates and the list shows a partial-result warning when `truncated` is true.
