...
- แท็บ `อัปเดตร่างล่าสุด` แสดง snapshot ร่างล่าสุดของผู้ดูและผู้ใช้อื่นในสาขาที่ผู้ดูมีสิทธิ์ แบบอ่านอย่างเดียวและแยกจากเอกสารจริง พร้อมคำบรรยาย `สิ่งที่เพิ่งบันทึกร่าง` ที่ server สรุปจาก diff ระหว่าง snapshot ก่อน/หลัง; ชื่อคู่ค้า/สินค้า/คลังที่แสดงต้อง resolve จาก active master ฝั่ง server ตาม trusted branch scope (หากหาไม่พบหรืออยู่นอก scope ให้ละเว้น) และห้ามแสดง `vehicleNo`, ชื่อจาก payload client หรือข้อความหมายเหตุ
...
| 2.2 | เปิดแท็บ `อัปเดตร่างล่าสุด` | poll snapshot ล่าสุดทุก 3 วินาทีเฉพาะขณะเปิดแท็บ/หน้า visible แล้วแสดง `เพิ่งบันทึกร่าง:` พร้อม `activityDescription` ที่ server derive จาก snapshot diff (เช่น เพิ่มเต๋า — ทองแดง, เปลี่ยนคลัง — ทองแดง → RM-A, แก้น้ำหนักชั่ง — ทองแดง · 125.50 กก.); ชื่อสินค้า/คลังในคำบรรยาย resolve จาก active master ฝั่ง server และไม่แสดง `vehicleNo`, ชื่อจาก payload client หรือข้อความหมายเหตุ พร้อมข้อมูลแบบอ่านอย่างเดียวของผู้ดูและผู้ใช้อื่นในสาขาที่มีสิทธิ์ |
...
- แท็บ `อัปเดตร่างล่าสุด` เป็นคนละ data surface กับ WTI/WTO: แสดง snapshot ล่าสุด ไม่ใช่สถานะธุรกิจ `draft` ของเอกสาร, ไม่มีปุ่ม Save/Edit/Confirm/Print/Share, และระบุ `เพิ่งบันทึกร่าง:` พร้อม `activityDescription`/เวลา เพื่อไม่อ้างว่าเป็น collaborative editing หรือ presence ระดับวินาที
