...
| ข้อมูล | รายละเอียด |
|---|---|
| เลขที่ใบรับเงิน | ออกเลข `RCP` ทันทีโดยใช้รหัสสาขา เช่น `RCP012607-0001` ไม่ใช้ข้อความรอออกเลข |
...
### 2.1 ลำดับการเลือกสาขาและ Sales Bill

สำหรับ RCP ที่รับเงินจาก Sales Bill ผู้ใช้ต้องเลือกสาขาก่อน แล้วระบบจึงแสดง Sales Bill ที่อยู่ในสาขานั้นให้เลือกต่อ การบังคับลำดับนี้ทำให้เลข RCP ใช้รหัสสาขาเดียวกับเอกสารต้นทาง และป้องกันการรวมบิลต่างสาขาใน Receipt Voucher เดียวกัน โดย API ตรวจสาขาซ้ำอีกชั้นก่อนบันทึกจริง

ตัวกรองสาขาบนตารางคิวรับเงินและตารางประวัติ RCP ใช้รหัสสาขาเดียวกันกับเอกสาร โดยกรองทั้ง Sales Bill ที่รอรับเงินและ RCP ที่บันทึกแล้ว ส่วน `CADV` จะถูกกรองจากสาขาที่บันทึกอยู่บน RCP header หากมีสาขา

กติกาเดียวกันใช้กับตารางเอกสารที่เลขที่มีรหัสสาขา ได้แก่ `PO Sell`, `Sales Bill`, `PB`, `PO Buy`, `ADJ`, `GA` และ `SC`: ตารางต้องมีตัวกรองสาขา และ API ต้องรับ/ตรวจ branch scope ก่อนคืนรายการ เพื่อไม่ให้การค้นหาแสดงเอกสารข้ามสาขา

สำหรับ `PMA` ให้ใช้สาขาจากเอกสารต้นทาง (`PB`, `ADV` หรือ `EXP`) โดย API ต้องกรองตั้งแต่ source query และหน้า list/detail ต้องแสดงสาขาเดียวกัน

### 2.2 กลุ่มเอกสารการเงินและโอนสินค้าที่ใช้รหัสสาขา

`EXP` และ `PADV` ต้องเลือกสาขาในฟอร์มก่อนบันทึก และใช้สาขานั้นออกเลขเอกสารรวมถึงตัวกรองตาราง ส่วน `PRET` ไม่ให้เลือกสาขาซ้ำ แต่ inherit จาก `PADV` ที่นำมาคืนเงิน เพื่อไม่ให้เอกสารต้นทางและเอกสารคืนเงินอยู่คนละสาขา

`RV` ต้องเลือกสาขาก่อนค้นหาบิลซื้อ และ inherit สาขาจากบิลซื้อที่เลือก เลข RV และตัวกรองรายการจึงอ้างอิงสาขาของบิลซื้อเสมอ การสร้าง RV แบบไม่อ้างอิงบิลซื้อที่มีสาขาไม่อยู่ใน contract นี้

`TRF` มีสาขาต้นทางและปลายทางอยู่แล้ว ใช้สาขาต้นทางเป็น branch code ของเลขเอกสาร และใช้สาขาต้นทางเป็นตัวกรองหลักในตาราง โดยยังคงแสดงสาขาปลายทางในรายละเอียดเพื่อให้ตรวจสอบเส้นทางการโอนได้ครบ
