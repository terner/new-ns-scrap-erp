...
- ลำดับหน้าจอคือ date scope filter -> compact cash plan -> actual activity KPI -> line tabs -> active detail surface
- cash plan แสดง `available`, `expected in`, `expected out`, และ `gap` ใน panel เดียว ห้ามทำ hero/banner ใหญ่หรือทำ KPI cards ซ้ำข้อมูลชุดเดียวกันข้างล่าง
- actual activity และ pending summary ต้องเป็น heading + grid ชั้นเดียว ห้ามใส่ KPI/stat cards อยู่ใน outer card อีกชั้น
- due AR, due AP, loan today, expense today, และ pending work เป็นคนละ line tab; อย่าวาง AR/AP สองตารางคู่กันจนคอลัมน์แคบ และอย่า stack ตารางทั้งหมดบนหน้าเดียว
- หัวตารางต้องเป็น Thai-first เช่น `ครบกำหนด` แทน `Due` และต้องไม่ย่อเป็น `...`
- หลีกเลี่ยง decorative emoji / giant success icon ในหัว panel, table title, KPI label, empty state, และ placeholder action ที่กดไม่ได้
...
- ใช้ date scope เดียวกับ `/api/owner-daily` ใน runtime ปัจจุบัน; branch/range filter ต้องเพิ่มเฉพาะเมื่อ API contract รองรับจริง
...
| 2 | เลือกวันที่ | API คำนวณ `ownerDaily` จาก source facts |
...
- `GET /api/owner-daily`
- implementation: dedicated `GET /api/owner-daily` route and owner-daily service; it does not re-export the dashboard overview handler
- permission: `reports.reports.view`

### Current Source Tables / Helpers

- owner-daily source scope is intentionally smaller than `/api/dashboard-overview`
...
- branch/range filter is not wired in the current `/api/owner-daily` runtime contract

## Implementation Checklist

- [x] Verify current API delegates to dashboard API
- [x] Document source response sections
- [x] Align rendered UI to compact owner-control design: date filter, non-duplicated cash plan panel, actual activity cards, and one detail surface per line tab
