...
- UI list action now opens SB cancel dialog and calls `PATCH /api/sales/bills/{docNo}`; server remains the source of truth for receipt-lock and reversal validation
...
- receipt relation/lock still needs end-to-end browser QA for SB cancel edge cases
