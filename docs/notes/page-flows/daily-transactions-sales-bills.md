...
- durable allocation tables/write-path for `sales_bill_lines`, `sales_bill_source_allocations`, `sales_bill_po_sell_allocations`, and `sales_bill_customer_advance_allocations` exists for new SB create/cancel; Stock SB detail/print/export still need a read-model normalization pass after reconciliation/backfill policy is defined
...
- [ ] Normalize Stock SB detail/print/export reads to durable allocation facts after migration/backfill policy
