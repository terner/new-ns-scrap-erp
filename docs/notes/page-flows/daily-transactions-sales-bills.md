...
- Trading SB ต้องเลือก PB Trading source ก่อนบันทึกสำหรับทุก PB-derived row โดย source เป็น `PB:<docNo>:<lineNo>` จาก Trading PB; manual `SRC:<sourceNo>:1` ยังเป็น backend-supported source แต่ไม่ใช่ primary create UX รอบนี้
