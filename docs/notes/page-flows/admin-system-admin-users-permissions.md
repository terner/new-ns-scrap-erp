...
As of 2026-08-03, `/admin/users` keeps one `จัดการ` ellipsis menu per row. Active users open one password dialog with two explicit choices: `ส่งลิงก์ตั้งรหัสผ่าน` (the user sets a password from email) or `สร้างรหัสผ่านชั่วคราว` (Admin sees it once and the user must change it after login). Pending users continue through the invite/reset API. If Supabase rejects an email, the Admin receives a specific message: per-email cooldown (wait about 60 seconds), project-wide email quota (wait about 1 hour), or a provider/SMTP failure without a guessed retry time. The public forgot-password flow remains generic to prevent account enumeration. Why it stays this way: credential commands remain row actions, while the authenticated Admin path can explain operational limits without exposing whether an arbitrary public email exists.
...
As of 2026-07-24, the create-user form checks the already loaded employee registry for an existing email before submitting `POST /api/admin/users`. If the email exists in `pending`, `active`, or `disabled`, the page stops the duplicate submit, switches the status filter/search to the existing record, and shows the current account status. What is what: `app_users.email` remains the employee identity key for this page, including pending invitations that are hidden by the default active-user filter. Why it stays this way: retrying create for an already invited employee must not create a second application profile or bypass the Supabase Auth invitation lifecycle; the administrator should continue from the existing row's `จัดการ` actions.
...
On 2026-07-24, local creation exposed a database drift: a later legacy migration had reintroduced `app_users.username` as `NOT NULL`, while the Prisma schema and email-only flow no longer wrote that field. The follow-up migration removes the legacy column again so creating a user does not require an obsolete identifier.

The contact phone field is optional, but when provided it accepts 8-15 digits with common `+`, space, hyphen, and parentheses formatting. The same validator runs in the form and both create/update APIs so invalid contact data cannot bypass the UI.

The email field is required and uses the same trimmed email schema in the form and both create/update APIs. Invalid email input shows the Thai message `รูปแบบอีเมลไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง` and the API returns a validation response instead of a generic server error.

The profile fields `ชื่อจริง` and `นามสกุล` are required for both create and update. This keeps the user identity shown in the employee list complete; the display name still composes from prefix, first name, and last name after validation.
...
PO Buy and PO Sell use dedicated permission resources instead of inheriting `finance.cash.view`. Each page registers `view`, `create`, `update`, `cancel`, and `short_close` actions under `purchase.po_buy.*` or `sales.po_sell.*`; the page/proxy checks `view`, mutation APIs enforce the matching action, and the list payload exposes capabilities so unavailable create/edit/cancel/short-close controls are not rendered. Migration `20260724020000_split_po_buy_sell_permissions.sql` copies existing role grants and direct allow/deny overrides from `finance.cash.view` to all ten new codes as a compatibility default. It was applied transactionally to dev-target on 2026-07-24; postflight confirmed all ten active catalog rows, 60 copied grants across six existing Roles, and the recorded migration-history row. Why it stays this way: PO commitments are purchase/sales operations, not cash visibility, and administrators must be able to show either page or permit each mutation without granting access to cash/bank data.
...
The employee list opens on `active` by default. After creating an employee, the page clears the status restriction to `all` instead of forcing `pending`, so the administrator keeps the current search context and sees the full list without an automatic post-save filter. The Admin can send the setup/reset link from the password dialog after the profile is committed. When an administrator activates a pending employee, the same dialog opens immediately. The server generates a strong password, stores it only in Supabase Auth, displays it once, and marks `must_change_password`. The raw password is never stored in application tables or audit metadata. Disabled users cannot receive credential actions.
