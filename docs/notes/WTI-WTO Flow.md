...
- child line ที่มี `parentId` ต้องมี `productId` ตรงกับ parent เสมอ; create/update WTI ต้อง reject payload ที่ผูก child คนละสินค้า และ calculator ต้องแยก child ที่ผิด contract ออกจากกลุ่มสินค้าแม้ถูกเรียกนอก request boundary
...
- ยอดหักของสิ่งเจือปนย่อยต้องถูกจำกัดอยู่ในสินค้าแม่ของตัวเองและห้ามนำ net ของสินค้าอื่นมารับยอดหักแทน ดังนั้น net ระดับสินค้าเท่ากับ `gross - หักภาชนะ - หักพื้นฐานของเต๋า - หักสิ่งเจือปนย่อย` โดยไม่ติดลบ
- header total ต้องใช้สูตรเดียวกับผลรวม product summary และ create/edit, detail, HTML print กับ React-PDF ต้องอ่านผลคำนวณชุดเดียวกัน เพื่อไม่ให้ยอดหัวเอกสาร ยอดแถวสีเขียว และยอดพิมพ์ต่างกัน
...
- แถวเต๋าในเอกสารพิมพ์ต้องแสดง raw lot arithmetic ของตัวเอง (`gross - หักภาชนะ - ยอดหักของเต๋า`) ส่วนสิ่งเจือปนย่อยและ final net ให้อยู่ในแถวสรุปสินค้าด้านล่างเพียงจุดเดียว เพื่อให้ HTML และ React-PDF อ่านเลขชุดเดียวกันโดยไม่ทำให้เลขรายแถวขัดกัน
