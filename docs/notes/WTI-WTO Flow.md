...
- [[WTO Return Flow]] เป็น owner ของ `WTO -> รับของคืน / return-loss / SB-CANCEL interaction`
...
- `WTI` และ `WTO` ห้ามใช้ logic pending_out/return ร่วมกัน: `WTI` ไม่มี `pending_out` และไม่มี action `รับของคืน` แบบ WTO; `WTI` ถูก consume โดย `PB` เพื่อเขียน stock-in ledger
- `WTO` เท่านั้นที่สร้าง/ปิด `pending_out`; action `รับของคืน` ในเอกสารนี้หมายถึงการปิดยอดค้างของ `WTO + สินค้า + คลัง`
...
### WTO Pending Out / SB Cancel / Return / Rebill Policy

Policy clarified on 2026-07-02:

- `WTO pending_out` เป็น cap ฝั่ง stock และ COGS เสมอ แม้ `SB` จะมี commercial qty มากกว่า source ที่ส่งจริง
- ตัวอย่าง `WTO = 100` และ `SB commercial qty = 120`: ตอน save `SB` ให้ consume pending_out และเขียน `stock_ledger.ref_type = SB` ได้สูงสุด 100 เท่านั้น; ส่วนเกิน 20 เป็นยอดการค้า/AR หรือ validation error ตาม policy หน้าบิลขาย แต่ห้ามสร้าง stock movement เกิน WTO source
- ถ้า cancel `SB` เดิมก่อนมี return/loss ให้ reverse จาก movement fact ที่เคย post จริง เช่น `SB-CANCEL qty_in = 100` ไม่ใช่ 120 และ reopen consumed pending_out กลับเป็น active 100
- หลัง cancel แล้ว WTO เดิมยังใช้เปิด `SB` ใหม่ได้ ตราบใดที่ pending_out ที่ถูก reopen ยัง active และยังไม่ได้ถูกปิดด้วย `รับของคืน` หรือ loss
- `รับของคืน` เป็น action ของ `WTO` ไม่ใช่ action ของ `SB`; `SB` เป็นเพียง downstream reference/audit ว่าเคย consume WTO นั้นไปออกบิล
- สิ่งที่รับคืนต้องอิงยอดค้างของ `WTO + สินค้า + คลัง` เป็นก้อนเดียว ไม่อิงเต๋า, line ย่อย, หรือ internal hold split
- ถ้าผู้ใช้เลือกปิดยอดด้วย `รับของคืน`: คืนครบให้ release active pending_out ของ `WTO + สินค้า + คลัง` โดยไม่เขียน stock-in ledger; คืนขาดให้ release pending_out ทั้งก้อนและเขียน `WTO-RETURN-LOSS` 1 row ต่อสินค้า+คลัง เฉพาะส่วนขาด; คืนเกิน pending_out ต้อง reject
- ถ้าเคยมี return/loss จาก `SB` นั้นแล้ว ภายหลัง cancel ต้องไม่ reopen/recreate pending_out ซ้ำ; ให้ `SB-CANCEL` คืน stock ตาม movement เดิมและคง return/loss audit เดิมไว้
- เมื่อ pending_out ของ WTO ถูกปิดด้วย `รับของคืน` หรือ loss แล้ว ห้ามนำ WTO เดิมไปเปิด `SB` ใหม่แบบปกติ ต้องออก WTO ใหม่หรือใช้ dedicated correction flow ที่ append audit ชัดเจน
- ปุ่ม/โมดัล `รับของคืน` ต้องแสดง 1 row ต่อ `WTO + สินค้า + คลัง`; ถ้าข้อมูลเก่ามีหลาย `stock_holds` จาก implementation เดิม ต้อง aggregate ก่อนคำนวณและก่อนเขียน business logs/ledger ห้ามเขียน movement หลาย row ตามเต๋า/hold/line
- target ใหม่ไม่ควรสร้าง `stock_holds` split ตามเต๋าสำหรับ WTO pending_out; pending_out ควรเป็นก้อนธุรกิจระดับ `WTO + สินค้า + คลัง` และเต๋าเป็นรายละเอียดเอกสาร/น้ำหนักเท่านั้น ไม่ใช่ stock reservation identity
...
| ราคาต้นทุนเฉลี่ย | ไม่ snapshot cost ในเอกสาร WTI | snapshot ราคาต้นทุนเฉลี่ยของ pending_out ตอน confirm หรือเมื่อมี delta เพิ่มระดับสินค้า+คลังหลัง confirm; เต๋าเป็น audit/detail ไม่ใช่ cost identity |
