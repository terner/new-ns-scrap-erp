...
- WTI เลือกคลังระดับ header จาก master data โดยส่ง `warehouses.id` จริง และ server ตรวจว่า active/อยู่สาขาเดียวกัน
- `weight_tickets.warehouse_id` เป็น relational source of truth; `warehouse_name` เป็น snapshot สำหรับเอกสารและประวัติ
- WTO ไม่มีคลังระดับ header เพราะต้องเลือก `warehouse_id` แยกตามสินค้า/line
- ข้อมูล WTI เก่าที่เป็น free text ต้อง map แบบ explicit ผ่าน `weight_ticket_warehouse_migration_issues`; ห้ามเดาจากชื่อ code หรือตัวเลขในข้อความ
