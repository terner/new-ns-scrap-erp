...
- แต่ก่อน save ยังต้อง validate ว่ามีสาขาและคู่ค้าถูกต้องตาม type
...
- `WTO` ต้อง validate จาก `พร้อมส่ง = คงเหลือจริง - จองไว้` และสร้าง stock hold เมื่อ save
- final stock movement ยังเกิดตอนบันทึก `SB` โดย consume hold ของ WTO
...
| `stock_holds` | มี durable hold table และ helper สำหรับ `active/released/cancelled/consumed`; SB create consume hold แล้ว | SB edit/cancel ยังต้องทำ reversal/reopen hold |
| `GET /api/daily/weight-tickets/options` | ส่ง `branches`, `suppliers`, `customers`, `impurities` | ไม่ต้องส่ง warehouse options รวม เพราะขึ้นกับ branch + product |
...
| 2 | เลือกสาขาและผู้ขาย | validate scope และคู่ค้า | draft |
...
| 2 | เลือกสาขาและลูกค้า | validate scope และคู่ค้า | draft |
| 3 | กรอกทะเบียนรถ/รูปรถ | เก็บหลักฐานเอกสารระดับ header | draft |
| 4 | เพิ่มรายการสินค้า | เลือกสินค้าก่อน แล้วเลือกคลัง `RM` / `FG` ใน line นั้นเพื่อดูคงเหลือและ validate stock | draft |
| 5 | กดบันทึก | ออกเลข `WTO...`, stamp วันเวลา/ผู้บันทึก, เขียน header/line/summary, intended warehouse ต่อ line, และ active stock hold โดยไม่เขียน stock ledger | `ส่งของแล้ว` / `จองสต็อกแล้ว` |
...
- ทุก line ของ `WTO` ต้องเก็บ intended warehouse และสร้าง hold เพื่อให้ `Sales Bill` ใช้เป็นคลังต้นทางในการตัด stock
- active hold ของ `WTO` ต้องลด available stock ทันที เพื่อกันการสร้าง WTO เพิ่มเกินยอดพร้อมส่ง
...
1. `warehouse / stock loop`
   - owner: หน้า `WTI/WTO` เฉพาะส่วนเก็บ intended warehouse, แสดง stock availability, และสร้าง/release stock hold
   - เกิดตอนสร้าง/แก้ไข/ยกเลิก `WTO`
   - รับผิดชอบ `product-first line entry`, `warehouse selection per line`, stock availability per product+warehouse, active hold, rebuild hold, และ release hold
...
- [x] เพิ่ม stock hold/reservation สำหรับ `WTO`: create/rebuild/release hold และแสดง `คงเหลือจริง / จองไว้ / พร้อมส่ง`
- [x] เติม `SB Stock` ให้ consume hold แล้วเขียน stock-out ledger โดยอ้าง `WTO` และ intended warehouse ตาม bill-driven target ใน create path
