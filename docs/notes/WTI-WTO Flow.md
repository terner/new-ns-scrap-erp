...
- WTI create/edit modal ต้องแสดง elapsed timer ใต้ title bar เพื่อบอกเวลาตั้งแต่เริ่มสร้างรายการจนถึงสถานะ `received` / `รับของแล้ว`; เอกสารที่บันทึกแล้วใช้ `weight_tickets.created_at` เป็นเวลาเริ่ม และยังนับต่อหลังบันทึก draft จนกว่าผู้ใช้กดปุ่มยืนยันรับของ ซึ่งเขียน `weight_ticket_status_logs.to_status = received` เป็นเวลาหยุดเท่านั้น ห้าม fallback ไปใช้ `updated_at` เพราะเวลานั้นอาจมาจากการแก้ไขอื่น ส่วน draft ที่ยังไม่บันทึกให้เริ่มนับจากเวลาที่เปิด modal เท่านั้น ไม่เพิ่ม field ใหม่ใน DB
