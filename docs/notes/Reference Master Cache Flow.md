...
- Corrected `audit:weight-ticket-image-assets` on 2026-07-19 found 214 valid JSON data URLs, 2 exact 15-byte PNG-declared mock payloads, and 23 filename-only references in both dev and SIT; missing canonical storage keys and migrated-namespace orphans remained zero. The previous no-data-URL result came from classifying JSON data URLs as filename-only. Runtime must not guess a storage path or add a base64/fallback image; valid assets remain dry-run, exact mocks require explicit CAS-safe removal, and filename-only rows remain pending source decisions. #151 has no list/picker attachment consumer, so this detail-only L5 migration keeps original assets only; thumbnail/compression remains a separate consumer-led migration.
