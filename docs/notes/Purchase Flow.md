...
Correction on 2026-05-30:
- หน้า `/purchase/payments` ยังต้องมี action `ยกเลิก` สำหรับ `รายการรอจ่าย` ที่ยังไม่เกิด payment จริง
- การกด `ยกเลิก` จาก `/purchase/payments` คือการยกเลิก approval snapshot รอบนั้นโดยเปลี่ยน `payment_approvals.status` เป็น `voided`
- หลังยกเลิก queue item แล้ว source document ต้องกลับไปอยู่ `/daily/payment-approval` เป็น `ยังไม่อนุมัติ`
- history ต้องเก็บ snapshot รายการที่ถูกยกเลิกในสถานะ `ยกเลิกแล้ว`
...
| อนุมัติแล้ว / รอออก PMT | `approved` 1..n rows | ไม่มี หรือมี `PMT pending` | ไม่ได้ | ไม่ได้ | `/purchase/payments` | บิลถูก lock แล้ว และอาจมีหลาย approval items ต่อ 1 เอกสาร; action คือ `ทำจ่าย` และ `ยกเลิก` |
| ยกเลิกรายการรอจ่าย | `voided` สำหรับ approval item ที่ถูกยกเลิก | ไม่มี | ได้ หลัง source กลับไป `ยังไม่อนุมัติ` | ได้ หลัง approval active หมด | `/purchase/payment-history` = `ยกเลิกแล้ว` และ `/daily/payment-approval` = `ยังไม่อนุมัติ` | ยกเลิก queue item ก่อนเกิด payment เท่านั้น |
| จ่ายสำเร็จ | `paid` หรือ `approved` ที่ถูก settle ครบ | มี `PMT success` | ไม่ได้ | ไม่ได้ | `/purchase/payment-history` = `เสร็จสิ้น` | downstream accounting ใช้เฉพาะแถวนี้ |
| ยกเลิกการจ่ายเงิน | `approved` ถ้ายังเหลือยอดค้าง, `paid` ถ้าถูก replace ด้วย PMT ใหม่ภายหลัง | มี `PMT cancelled` | ไม่ได้ | ไม่ได้ | `/purchase/payment-history` = `ยกเลิกแล้ว`, และถ้ายังเหลือยอดค้างต้องเห็นต่อใน `/purchase/payments` | ยกเลิกที่ PMT เกิดหลังมี payment แล้ว |
...
5. กดยกเลิก `PMT`:
   - เป็นการยกเลิก payment execution/voucher ไม่ใช่ยกเลิก approval
   - history ต้องเห็นรายการนั้นเป็น `ยกเลิกแล้ว`
   - approval ต้นทางไม่ถูกเปลี่ยนเป็น `voided`
   - ถ้ายอด approved ของ item นั้นยังไม่ถูก settle ครบ ระบบต้องให้กลับมาอยู่ใน `/purchase/payments` ต่อ

6. ประวัติการจ่ายเงิน:
