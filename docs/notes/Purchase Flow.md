...
- อัปเดต `WTI`
- ลง `stock_ledger` เฉพาะกรณี Stock
...
flowchart TD
  ...
  PoWtiRecalc["recalc PO/WTI<br/>ออกบิลบางส่วน / ออกบิลแล้ว / เสร็จสิ้น"]
...
| `UC-PUR-09` | เลือก WTI แล้วเปลี่ยนสาขา/คลัง/ผู้ขาย/ใบรับของกลางทาง | ใช่ | `Lock ต้นทางหลังเลือกใบรับของ` | ครบระดับ UX/control rule |
...
| 2 | จ่ายเงินล่วงหน้า / มัดจำตาม PO (optional) | Supplier, สาขา, วันที่จ่าย, วิธีจ่าย, บัญชีที่จ่าย, ยอดจ่ายล่วงหน้า, เอกสารอ้างอิง PO/ใบชั่งน้ำหนักใหญ่ถ้ามี | `ADV...` | ADV = `บันทึกแล้ว` | เป็น source แยกของ [[Payment Flow]]; Purchase Flow ใช้ได้เฉพาะยอด ADV ที่จ่ายจริงแล้วและยัง available |
| 3 | Supplier เอาของมาส่ง | ยังไม่กรอกในระบบ | ไม่มี | PO ยัง `เปิดอยู่` | รอรับของจริง |
| 4 | บันทึกใบรับของ / WTI | ประเภท `Stock`, เลือก Supplier, สาขา, ทะเบียนรถ, สินค้า, น้ำหนัก, วิธีหักสิ่งเจือปน, รูปต่อรายการสินค้า/หมายเหตุ | `WTI...` | ใบรับของ = `รับของแล้ว` | บันทึกยอดรับจริงเข้า stock รอ office ออกบิล |
| 5 | Office สร้างบิลรับซื้อ Stock | เลือกสาขา, ผู้ขาย, และใบรับของที่ยังไม่ออกบิลครบ | ไม่มีจนกว่าจะ save | Draft PB | ระบบดึงรายละเอียดสินค้าและน้ำหนักจากใบรับของ |
...
| 9 | เลือกมัดจำที่จะหักกับบิล (ถ้ามี) | เลือก `ADV` ของ supplier/branch เดียวกัน และระบุยอดที่จะ allocate เข้าบิล | ไม่มี | Draft PB | ยอดเจ้าหนี้สุทธิ = ยอดบิลเต็ม - ยอด ADV ที่ allocate; ห้ามใช้ ADV ซ้ำเกินยอดจริง |
...
| 11 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `เสร็จสิ้น` สำหรับยอด WTI ที่เลือกครบในบิลนี้ | กันการนำใบรับของไปออกบิลซ้ำเกินยอด; สถานะ partial ใช้เฉพาะกรณีมี active usage เดิมที่ยังไม่ครบ |
| 12 | ระบบอัปเดต PO จากบิล | ไม่ต้องกรอกเพิ่ม | ไม่มี | PO = `ออกบิลบางส่วน` หรือ `ออกบิลแล้ว` | ตัด PO เฉพาะยอดที่ allocate เป็น PO |
| 13 | ระบบลง stock ถ้าเป็นบิลเข้าสต๊อก | ไม่ต้องกรอกเพิ่ม | อาจเกิด stock ledger id ภายใน | Stock = `บันทึกแล้ว` | เพิ่ม stock และต้นทุน; ถ้าเป็น line ที่อ้าง PO ให้ reconcile PO Buy candidate เดิม, ถ้าเป็น Spot Buy ทองแดง/ทองเหลืองจึงเข้า Cost Pool |
...
| 16 | ยกเลิกบิลรับซื้อ (ถ้ายังไม่มี payment cycle active) | กรอกเหตุผลการยกเลิก | ไม่มี | PB = `ยกเลิก` | ลบผลกระทบ stock ledger ของบิล, recalc การใช้งาน WTI ใหม่จากบิลที่ยัง active อยู่, และคืนยอด ADV allocation ให้ใช้ได้ตาม policy |
...
| `อนุมัติแล้ว` | Payment Flow มี approval snapshot ของ PB แล้ว | lock field การเงินและ allocation |
| `รอจ่าย` | Payment Flow มีรายการที่รอจ่ายจริง | lock field การเงินและ allocation |
...
| 4 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `เสร็จสิ้น` สำหรับ WTI ที่เลือกครบในบิลนี้ | บิลใหม่ห้ามปล่อยยอด WTI ที่เลือกค้างจัดสรร; สถานะ `ออกบิลแล้วบางส่วน` ใช้กับข้อมูลเดิมหรือ WTI ที่เหลือจาก active PB อื่นเท่านั้น |
| 5 | ระบบลง stock ถ้าเป็นบิลเข้าสต๊อก | ไม่ต้องกรอกเพิ่ม | อาจเกิด stock ledger id ภายใน | Stock = `บันทึกแล้ว` | เพิ่ม stock และต้นทุน; ถ้าเป็นทองแดง/ทองเหลืองให้เข้า Cost Pool ในฐานะ Spot Buy / No PO |
...
| 2 | จ่ายเงินล่วงหน้า / มัดจำตาม PO (optional) | Supplier, สาขา, วันที่จ่าย, วิธีจ่าย, บัญชีที่จ่าย, ยอดจ่ายล่วงหน้า, เอกสารอ้างอิง PO/ดีล | `ADV...` | ADV = `บันทึกแล้ว` | เป็น source แยกของ [[Payment Flow]]; Purchase Flow ใช้ได้เฉพาะยอด ADV ที่จ่ายจริงแล้วและยัง available |
| 3 | หน้างานระบุว่าเป็น Trading + PO | ระบุว่าอ้าง PO ไม่ใช่ Spot | ไม่มี | ไม่มี | ไม่มีใบรับของและไม่เข้า stock |
| 4 | Office ออกบิลรับซื้อ Trading | ประเภท `Trading`, แหล่งซื้อ `PO`, Supplier, PO/PO line, สินค้า, จำนวน/น้ำหนัก, ราคา, VAT, หมายเหตุ, ADV ที่ต้องนำมาหักถ้ามี | `PB...` | PB = `เปิดอยู่` หรือ `เสร็จสิ้น` ถ้า ADV ตัดเต็ม | ตั้งเจ้าหนี้/AP สุทธิหลังหัก ADV และตัด PO ตามจำนวน/น้ำหนักที่กรอกในบิล |
...
Open question / wording: ยังไม่ควรถือว่า `WTI` มีสถานะ `รับของแล้วบางส่วน` เป็น target behavior จนกว่าจะ confirm เพิ่ม ถ้าคำนี้ยังเจอใน UI/API ต้องแยกให้ชัดว่าเป็นสถานะ `POB` ที่รับของจาก supplier บางส่วน หรือเป็น wording ผิดของ `WTI` สำหรับ `WTI` partial target ชั่วคราวคือ `ออกบิลแล้วบางส่วน` จากการนำใบรับของไปออก PB บางส่วน ไม่ใช่การรับของบางส่วน และยังต้องตัดสินใจว่าจะปิดสถานะ partial ของ WTI ใน new write path ทั้งหมดหรือเหลือไว้เฉพาะ legacy/active partial usage หลัง recalc
...
| `อนุมัติแล้ว` | มียอดที่ถูก approve เป็น PMA แล้ว |
