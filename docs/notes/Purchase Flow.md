...
| `UC-PUR-06` | ใบรับของ 1 ใบตัดหลาย PO | ใช่ | `กติกา allocation` ของ Stock + PO | ครบระดับ rule แต่ UI split line ยังเป็น follow-up |
...
| `UC-PUR-14` | จ่ายเงิน supplier หลังออกบิลซื้อ | ใช่ | `ใบจ่ายเงิน Supplier` | ครบระดับ business flow |
...
| 6 | Allocate ใบรับของเข้ากับ PO/Spot | เลือก PO เพื่อเอาจำนวน/น้ำหนักไปตัด PO; ถ้าใบรับของเกิน PO ให้กด `เพิ่มสินค้า` เลือกใบรับของเดิม แล้วเลือก `Spot Buy` สำหรับยอดส่วนเกิน | ไม่มี | Draft PB | แยกยอดใบรับของเดียวกันเป็น PO และ Spot ได้จนยอดครบใบรับของ |
...
| 9 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `ออกบิลบางส่วน` หรือ `ออกบิลแล้ว` | กันการนำใบรับของไปออกบิลซ้ำเกินยอด |
...
| 12 | จ่ายเงิน Supplier | เลือกบิลค้างจ่าย, บัญชีจ่าย, วิธีจ่าย, ยอดจ่าย, WHT/ส่วนลด, หมายเหตุ | `PMT...` | Payment = `บันทึกจ่ายแล้ว` | บันทึกเงินออกและตัดเจ้าหนี้ |
| 13 | ระบบอัปเดตบิลรับซื้อ | ไม่ต้องกรอกเพิ่ม | ไม่มี | PB = `จ่ายบางส่วน` หรือ `จ่ายครบ` | ยอดค้างจ่ายลดลง |

กติกา allocation:

- ใบรับของ 1 ใบสามารถถูกแตกในบิลรับซื้อไปหลาย PO ได้ ถ้าเป็น Supplier เดียวกันและสินค้า/เงื่อนไขตรงกัน
- ใบรับของ 1 ใบสามารถมีบางส่วนเป็น `PO` และบางส่วนเป็น `Spot Buy` ได้ เมื่อยอดรับจริงมากกว่า PO ที่เลือกมาตัด
...
| 4 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `ออกบิลแล้ว` | กันการออกบิลซ้ำ |
...
| สถานะ | WTI: `รับของแล้ว`, `ออกบิลบางส่วน`, `ออกบิลแล้ว`, `ยกเลิก`; WTO: `ส่งของแล้ว`, `ออกบิลบางส่วน`, `ออกบิลแล้ว`, `ยกเลิก` |
...
- ถ้าน้ำหนักในใบรับของมากกว่า PO ที่เลือกมาตัด ผู้ใช้ต้องกด `เพิ่มสินค้า`/เพิ่มบรรทัด เลือกใบรับของเดิม แล้วเลือก `Spot Buy` สำหรับยอดส่วนเกิน หรือเลือก PO อื่นที่ยังเหลือยอด
- ใบรับของที่นำมาออกบิลในรอบนั้นต้องถูก allocate ครบระหว่าง `PO` และ `Spot Buy` ก่อนบันทึกบิล
...
| `ออกบิลแล้ว` | น้ำหนักในใบรับของถูกนำไปออกบิลครบแล้ว |
...
- ต้องเลือกใบรับของที่สถานะ `รับของแล้ว` หรือ `ออกบิลบางส่วน`
- ห้ามเลือกใบรับของที่ `ออกบิลแล้ว` หรือ `ยกเลิก`
- ใบรับของที่เลือกต้องตรงกับสาขาและ Supplier ของบิล
- ระบบต้องแสดงสินค้าและน้ำหนักจากใบรับของ และรายการ Stock ต้องอ้าง receipt line เดิม
...
- ถ้ายอดใบรับของเกิน PO ที่เลือก ผู้ใช้ต้องกด `เพิ่มสินค้า` เลือกใบรับของเดิม และเลือก `Spot Buy` หรือ PO อื่นสำหรับยอดส่วนเกิน
- ผลรวม allocation ของ receipt line เดียวกันในบิลต้องเท่ากับยอดใบรับของที่นำมาออกบิล และต้องไม่เกินยอดยังไม่ออกบิล
...
|---|---|
| PO ปัจจุบัน | `po_buys` |
| ยอดรับจริงเข้า stock | `weight_tickets` + `weight_ticket_items` ที่มีเลขเอกสาร prefix `WTI` |
...
| รูปภาพหลักฐานต่อรายการสินค้า | `weight_ticket_item_images` หรือ attachment table equivalent ที่ผูกกับ `weight_ticket_items`; ต้องมีอย่างน้อย 1 รูปต่อ 1 รายการสินค้า |
| วิธีหักสิ่งเจือปน | `weight_ticket_items` หรือ line table equivalent: Gross, deduction mode, impurity reference, deduction weight/percent, Net |
...
- ใช้ target table กลางสำหรับเอกสารชั่งน้ำหนัก เช่น `weight_tickets` + `weight_ticket_items` โดยแยกขาเข้าด้วยเลข `WTI` และขาออกด้วยเลข `WTO`; ไม่ใช้เลข `WT` เดี่ยว
