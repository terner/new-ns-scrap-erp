...
- รับ FG/RM/Customer Return กลับเข้าคลัง
- บันทึก loss/yield/cost allocation

## Target Production Lifecycle

| Step | Document/Action | Meaning | Stock ledger |
|---|---|---|---|
| 1 | Create Production Order | เปิดงานผลิต, ระบุ branch, machine, line, target product, planned qty | ยังไม่กระทบ stock |
| 2 | Approve / Start | อนุมัติให้เริ่มผลิต | ยังไม่กระทบ stock |
| 3 | Production Input | เบิกวัตถุดิบเข้า WIP | paired movement `PI`: RM out + WIP in |
| 4 | Process Cost | บันทึกค่าแรง/ไฟ/เครื่องจักร/ค่าใช้จ่ายผลิต | ไม่กระทบ stock qty; ใช้คำนวณ cost |
| 5 | Production Output | รับผลผลิต / loss | `PO2`: WIP out + destination in หรือ loss |
| 6 | Cost Allocation | กระจายต้นทุน input + process cost ไป output | update cost/value ตาม policy |
| 7 | Complete | จบงานผลิตเมื่อ input/output/WIP ถูกตรวจแล้ว | lock normal mutation |
| 8 | Close / Lock Cost | ปิดต้นทุน | ห้ามแก้ไข ยกเว้น reversal flow |
...
- edit input ต้อง reverse/rebuild หรือ controlled delete/rebuild ใน transaction
- reverse input ต้องคืน WIP กลับ source stock ด้วย paired reverse movement
...
| `CUSTOMER_RETURN` | `PRODUCTION_OUTPUT_WIP_OUT` | `CUSTOMER_RETURN_IN` | no |
...
- `CUSTOMER_RETURN` ต้องเข้า stock แบบ `not_available_for_sale = true`
- output category ต้องมาจาก active `production_output_categories`
- output ที่เปลี่ยน product/grade ต้องเชื่อมกับ Grade Convert หรือ cost allocation ให้ trace ได้
...
| `CUSTOMER_RETURN` | ของคืนลูกค้า | `return_stock_in` |
...
- ยังไม่มี `POST/PATCH` write flow สำหรับ production order/input/output/process cost
...
- [ ] Approval/start/complete/close status flow
- [ ] Production Input write + paired `PI` stock ledger
- [ ] Production Output write + `PO2` stock ledger
- [ ] Process Cost write and allocation policy
- [ ] Reverse input/output policy
- [ ] WIP reconciliation report from ledger vs production tables
- [ ] Cost allocation recompute policy after edit/reverse
- [ ] Lock rules after `Completed` / `Closed`
...
- WIP balance = sum(`WIP_IN`) - sum(`PRODUCTION_OUTPUT_WIP_OUT`) - sum(`PRODUCTION_LOSS`) - WIP return out/in ตาม policy
...
- Closed order ต้องไม่มี WIP คงเหลือ เว้นแต่มี reason/exception
