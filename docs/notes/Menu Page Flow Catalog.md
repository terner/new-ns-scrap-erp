...
| `/stock/status-convert` | ปรับสถานะสินค้า (RM→FG) | [[Stock Status Convert Page Flow]] | accepted code baseline | target `statusConvert`: RM/FG status conversion with paired `SC` stock ledger and source WAC | finish reversal policy |
