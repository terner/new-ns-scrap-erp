...
| `/dashboard-overview` | Dashboard Overview | [[Main Dashboard Reports Flow]], [[page-flows/main-dashboard-reports-dashboard|Dashboard Page Flow]] | accepted code baseline | current `/api/dashboard-overview`: management KPI overview from PB/SB/EXP/PMT/RCP/stock/bank/production/historical facts | reconcile KPI formulas with Finance Accounting dashboards |
...
| `/sales-plan` | วางแผนการขาย (LME) | [[Sales Flow]] | accepted code baseline | legacy `salesPlan`: propose sale channel/LME percentage and lock selling plan | connect locked plan to PO Sell / WTO-backed Sales Bill flow |
...
| `/sales/bills` | บิลขาย | [[Sales Bills Page Flow]], [[Sales Flow]] | accepted code baseline | target: `WTO -> SB -> RCP`, PO Sell allocation, Spot Sale split, and no PSALE source for new writes | finish allocation facts, stock-out rules, and receipt relation |
| `/sales/stock-issue` | เบิกออกรอบิล | [[Pending Sale Page Flow]] | removed target flow | legacy `stockIssue`/`PSALE` reference only; active runtime route/API is removed/410 | replace old QA/report references with WTO-backed SB coverage |
...
| `/finance/ar` | ลูกหนี้ (AR) | [[Finance Debt Flow]], [[Finance AR Page Flow]], [[page-flows/finance-debt-finance-ar|AR Page Flow]] | accepted code baseline | target: AR visible balance reads `sales_bills.receivable_balance` / `received_amount`; RCP/Customer Advance are drilldown facts; no Pending Sale summary | switch API to snapshot-first balance, add source document links, created-date display, customer advance allocation facts |
...
| `/stock/status-convert` | ปรับสถานะสินค้า (RM/FG) | [[Stock Status Convert Page Flow]] | accepted code baseline | target `SC`: RM<->FG stock reclassification for wrong classification, quantity bucket update, no WAC reprice, no production/yield effect, and Production reporting source label | finish hold-aware validation and reversal policy |
...
| `/finance-accounting/accounting-periods` | Accounting Periods | [[Finance Accounting Flow]], [[Finance And Accounting Menu Summary]], [[page-flows/finance-accounting-finance-accounting-accounting-periods|Accounting Periods Page Flow]] | FA5 policy UI baseline | policy surface for period state, soft close, lock, reopen, and year close readiness | define DB period model and runtime closed-period enforcement |
| `/finance-accounting/posting-rules` | Posting Rules | [[Finance Accounting Flow]], [[Finance And Accounting Menu Summary]], [[page-flows/finance-accounting-finance-accounting-posting-rules|Posting Rules Page Flow]] | FA5 policy UI baseline | policy surface for source-to-account mapping readiness before GL/statutory posting | define rule schema/versioning/audit and GL posting boundary |
