...
| Financial LINE notification | `docs/notes/Purchase and Sales Bill LINE Notification.md` | post-commit notification contract for `/purchase/bills`, `/sales/bills`, `/purchase/payments`, and `/sales/receipts`; explicit `PB`/`SB`/`PMT`/`RCP` group routing, Flex Message fields, fail-closed delivery, queue/retry behavior, and configuration checklist |
...
| Customer advance receipt flow | `docs/notes/Customer Advance Receipt Flow.md` | canonical target `CADV -> RCP -> SB`: Packing List source form, customer receipt integration, allocation boundaries, target API and data model |
| Customer advance retired page baseline | `docs/notes/Customer Advance Page Flow.md` | historical read baseline for retired `/finance/customer-advance`; not the target CADV write contract |
