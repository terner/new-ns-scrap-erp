...
| Stock transfer page flow | `docs/notes/Stock Transfer Page Flow.md` | page-specific contract for `/stock/transfer`: paired `ST` ledger movement, source/destination validation, availability check, and reverse/cancel policy gap |
| Stock status convert page flow | `docs/notes/Stock Status Convert Page Flow.md` | page-specific contract for `/stock/status-convert`: paired `SC` RM/FG status conversion, ready-stock validation, source WAC carry-forward, and reconciliation/reverse policy gap |
| Stock convert page flow | `docs/notes/Stock Convert Page Flow.md` | page-specific contract for `/stock/convert`: `GA` grade/product conversion, source-target quantity/cost policy, and deferred confirm/reverse behavior |
| Stock adjust page flow | `docs/notes/Stock Adjust Page Flow.md` | page-specific contract for `/stock/adjust`: `ADJ` stock count adjustment, note-only accounting policy, required reason/audit, and reverse policy gap |
...
| Finance Debt flow | `docs/notes/Finance Debt Flow.md` | category-level Finance & Debt source of truth for active menu pages: Petty Advance, AR, AP, Bank Statement, Cash Position, Customer Advance, current APIs, legacy baseline, side-effect boundaries, and open allocation/status gaps |
...
| Finance AP page flow | `docs/notes/Finance AP Page Flow.md` | page-specific contract for `/finance/ap`: AP aging from purchase bills/payments, due-date rules, supplier summary, export, and payment allocation/source-link gaps |
| Finance AR page flow | `docs/notes/Finance AR Page Flow.md` | page-specific contract for `/finance/ar`: AR aging from sales bills/receipts, customer credit term/due-date rules, pending issue summary, and customer-advance allocation gap |
...
| Finance Accounting flow | `docs/notes/Finance Accounting Flow.md` | category-level Finance / Accounting source of truth for 19 active menu pages: current APIs, query params, source builders/tables, write-disabled design states, management-report boundary, and GL/statutory/tax/asset/loan gaps |
| Trading flow | `docs/notes/Trading Flow.md` | category-level Trading / PO Reports source of truth for active menu pages: Trading Dashboard, Trading Matching, PO Outstanding, ex-VAT GP, PB/SB Trading matching, PO commitment report, current APIs, legacy baseline, and no-stock-ledger side-effect boundary |
...
| Pending sale page flow | `docs/notes/Pending Sale Page Flow.md` | page-specific contract for `/sales/stock-issue`: physical stock issue before billing, `PSALE` stock-out, convert-to-SB without duplicate stock cut, and current read-only/list implementation gap |
| PO Sell flow | `docs/notes/PO Sell Flow.md` | page-specific contract for `/sales/po-sell`: create status, match status, downstream visibility in Pending Sales, Cost Allocator, PO Outstanding, Sales Bill/Stock Issue handoff, and current gaps |
