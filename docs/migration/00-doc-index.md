...
| Purchase/Sales Bill LINE notification | `docs/notes/Purchase and Sales Bill LINE Notification.md` | create-success notification contract for `/purchase/bills` and `/sales/bills`, explicit `PB`/`SB` group routing, Flex Message fields, fail-closed delivery, queue/retry behavior, and configuration checklist |
