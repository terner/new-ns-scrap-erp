...
| Stock status convert page flow | `docs/notes/Stock Status Convert Page Flow.md` | page-specific contract for `/stock/status-convert`: paired `SC` RM/FG status conversion, ready-stock validation, RM<->FG validation, source WAC carry-forward, Production reporting source-label boundary, and reconciliation/reverse policy gap |
