...
- DB/API changes: added `POST /api/purchase/po-buy` using existing `po_buys` fields only; no schema migration. New rows use the generated `doc_no` as both business document number and meaningful `id` for created PO Buy rows. PO issue time is stamped server-side at save time in `created_at` / `updated_at`; `date` remains date-only for existing report/filter compatibility and the running `POBYYMM` number uses the Bangkok date derived from that timestamp. The create payload no longer needs a client-owned issue date, uses one optional `warehouseId` for `สาขา/คลัง`, derives `branch_id` from the selected warehouse, and leaves `channel_id` null because purchase channel duplicates branch/location in this flow. Existing UUID-backed legacy rows remain readable.
- Validation added: shared Zod schema validates supplier, optional `สาขา/คลัง` (`warehouseId`), optional delivery date, notes length/syntax, and 1-50 item rows with product/positive quantity/positive unit price. Server verifies active Supplier/Product/Warehouse before insert and compares delivery date to the server-owned PO issue date.
- UI behavior: enabled the existing blue `+ PO Buy ใหม่` CTA and added the legacy-style modal with amber purpose selector, delivery/costing radio cards, 2-column header fields, one `สาขา/คลัง` selector, multi-line item table, live totals, remove row behavior only when more than one row exists, note field, and blue save button. The modal intentionally hides PO issue date/document number and removes `ช่องทางรับซื้อ`.
...
- `/master-data/salespersons` code generation/validation now uses lowercase `s001`-`s999` sale codes. Manual values such as `S1`/`s1` normalize to `s001`; values outside the range are rejected before write.
