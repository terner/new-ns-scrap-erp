...
- Current supplier import follow-up: `/master-data/suppliers` is gaining an Excel import path that accepts the same `.xlsx` workbook produced by the supplier export sheet `ผู้ขาย`. Import is all-or-nothing, validates rows before write, maps `ผู้ดูแล` back to active salespersons by id/code/name, uses uppercase `SU...` as both `id` and `code`, and keeps address data in form/export/import while hiding the address column from the on-screen supplier table.
