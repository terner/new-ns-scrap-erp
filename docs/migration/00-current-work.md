...
- Current `ใบรับของ` detail update: per user clarification on 2026-05-25, the receipt document must auto-stamp document number, document date, creation time, and entered-by user from system/auth data only after save; the form must not preview or display those generated values before save. The operator enters vehicle plate, selects branch/product, enters weight, chooses impurity deduction mode (`ไม่หัก`, `หัก`, `หัก%`), selects an active impurity from `/master-data/impurities` when the line has a deduction, adds image evidence per product line, and then the system issues the receipt document. `/daily/weight-tickets` now implements this as a UI/localStorage prototype for WTI/WTO with simplified labels such as `ผู้ขาย`, red required asterisks, no Ready Check / after-save helper aside panel, no sticky top weight summary, a bottom-positioned `เพิ่มสินค้า` button after the item list, and a fixed save footer that summarizes item count/gross/deduction/net while offsetting on desktop so it does not overlap the app sidebar; no OpenAPI, DB schema, or server persistence has been added yet.
- Current line-image validation update: per user clarification on 2026-05-25, each product line in WTI/WTO must have at least one image before saving. Images are no longer treated as one document-level attachment bucket; the prototype stores image count/names on each product line and aggregates totals at the document header for listing.
- Current vehicle-image follow-up: per user request on 2026-05-25, `/daily/weight-tickets` now has document-level `รูปภาพรถส่งของ` upload beside the `ทะเบียนรถ` field. Vehicle images are stored separately from product-line images in the localStorage prototype, image filenames can open a preview dialog, and product image rows also have an `เปิดรูปภาพ` action. When deduction mode is `ไม่หัก`, the deduction value input is hidden instead of disabled.
- Current `รายการใบรับ-ส่งของ` implementation update: `/daily/weight-ticket-list` now exists as a UI/localStorage prototype. It lists WTI/WTO records created from `/daily/weight-tickets`, includes sample rows when local storage is empty, supports search/type/status/branch/date filters, opens a detail modal, and shows image count per product line. This remains a prototype until target `weight_tickets` tables and API routes are designed.
