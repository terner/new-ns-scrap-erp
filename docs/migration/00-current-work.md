...
- Current master-data follow-up: `/master-data/salespersons` now uses sale codes in the lowercase `s001`-`s999` format. The API normalizes manual input like `S1`/`s1` to `s001`, generates the next `sNNN` code, and rejects values outside the `s001`-`s999` range.
