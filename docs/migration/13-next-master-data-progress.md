...
- Supplier receiving account display uses separated `bank_name` and digit-only `bank_account`. Supplier modal stores receiving accounts in child rows with payment method first: `เงินสด` requires no bank/account data, while `โอนเงิน` requires a bank and digit-only account number before save. Import strips cash markers such as `เงินสด` from bank/account text and splits multiple account numbers into separate `supplier_bank_accounts` rows.
