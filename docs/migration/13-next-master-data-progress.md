...
4. ดึงข้อมูลจริงจาก `production` ผ่าน Prisma
...
- Prisma schema has been introspected from `production`.
...
- Batch 1 pages now have real Next routes, shared master-data list/form UI, API routes, Prisma/production reads, add/edit baseline, search/sort, and active/inactive where the source table supports it:
  - `/master-data/salespersons`
  - `/master-data/currencies`
  - `/master-data/expense-categories`
  - `/master-data/channels`
- Batch 2 pages now have real Next routes, shared master-data list/form UI, API routes, Prisma/production reads, add/edit baseline, search/sort, and active/inactive:
  - `/master-data/branches`
...
- Batch 4 pages now have real Next routes, shared master-data list/form UI, API routes, add/edit baseline, search/sort, and active/inactive:
  - `/master-data/beneficiaries` uses real Prisma/production table `overseas_recipients`.
...
- Thai address reference tables exist in `production`: provinces 77, districts 928, subdistricts 7,436.
...
| 2026-05-17 | Supabase `db push` to `production` | Passed | Additive-only migration applied; no destructive DDL/DML |
...
| 2026-05-17 | Supplier standardization: `npm run lint`, `npm run type-check`, `npm run build` | Passed | Supplier master follows customer-style form/list/export pattern; supplier classification migration applied to production |
...
| 2026-05-18 | Additive bank account field split migration on `production` | Passed | Added `bank_name`/`account_no` to `director_employees` and `payment_methods`; backfilled 3 director account rows from old `bank_account`; no rows or old columns deleted |
...
| 2026-05-18 | Additive product type/unit master migrations on `production` | Passed | Created `product_units` with 2 seed rows and `product_types` with 12 rows including `อิเล็กทรอนิกส์`; no product rows changed or deleted |
...
| 2026-05-18 | Bank names submenu integration: `npm run prisma:generate --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `npm run build` | Passed | Created `bank_names` in `production` with 2 valid seed rows; `/master-data/bank-names` and API routes are included in the Next build |
| 2026-05-18 | Supplier contact removal + finance reference cleanup in `production`: `npm run prisma:generate --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `npm run build` | Passed | Dropped supplier contact columns after backup of 1,841 rows; normalized bank/currency code+symbol data; cleared payment method bank/account values; verified supplier contact columns no longer exist |
| 2026-05-18 | Branch HQ row removal in `production` | Passed | Removed `public.branches` row `BR001/HQ/สำนักงานใหญ่`; backed up the branch row plus 15 referencing rows to `maintenance.branch_hq_removal_backup_20260518`; cleared nullable `branch_id` references in accounts, expenses, purchase bills, stock ledger, and warehouses |
| 2026-05-18 | Supplier email removal + receiving account columns in `production`: `npm run prisma:generate --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `npm run build` | Passed | Dropped `public.suppliers.email` after creating `maintenance.supplier_email_backup_20260518`; supplier table/form/export now use receiving bank/account fields instead of email |
| 2026-05-18 | Supplier bank account split + Thai commercial bank seed in `production` | Passed | Split bank names out of supplier account numbers for 1,927 rows; backup stored in `maintenance.supplier_bank_account_split_backup_20260518`; seeded 14 bank names including TTB, Krungsri, Thai Credit, UOB, ICBC, and Standard Chartered |
...
| 2026-05-20 | Supplier international address + bank dropdown follow-up | Passed | Added international supplier address columns to production, moved domestic/foreign selection into the address section, added foreign address line/city/state/postal/country-code fields, removed the general supplier note from modal/export/import, kept `ที่อยู่เต็ม/หมายเหตุที่อยู่`, and changed receiving bank to a dropdown from `bank_names`; `npm run lint --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `git diff --check`, and `npm run build --workspace @ns-scrap-erp/next` passed |
| 2026-05-20 | Supplier CSV replacement + bank/tax repair follow-up | Passed | Replaced production supplier data from `docs/tasks/assets/reference/nsscrap permission and master data   - ผู้ขาย.csv`; final counts are suppliers `1888`, active CSV rows `1871`, inactive preserved referenced rows `17`, missing owners `0`, FK orphans `0`; added missing bank masters, restored 97 bank/account rows from backup where CSV was blank and name matching was reliable, and cleared all supplier tax IDs for user keying. Backups: `maintenance.supplier_replace_backup_20260520072518`, `maintenance.supplier_bank_tax_repair_backup_20260520082749`, and `maintenance.supplier_tax_clear_backup_20260520082854`. Modal/import now allow blank tax ID and hide `รหัสประเทศ (ISO)`. `npm run lint --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `git diff --check`, and `npm run build --workspace @ns-scrap-erp/next` passed |
...
| 2026-05-20 | Currency/remittance code removal + submenu-only parents | Passed | Product and machine sidebar parent rows are now submenu toggles only, with working child labels `รายการสินค้า` and `รายการเครื่องจักร`. Production `currencies.code` and `overseas_remittance_purposes.code` were removed after backups in `maintenance.currency_code_removal_backup_20260520131607` and `maintenance.remittance_purpose_code_removal_backup_20260520131607`. Currency keeps internal `id` derived from uppercase `symbol`; foreign-finance APIs still expose a compatibility `code` value from `symbol` for current forms. Validation passed: `npm run prisma:generate --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `git diff --check`, and `npm run build --workspace @ns-scrap-erp/next`. |
| 2026-05-20 | Accounts/machines/production-lines code removal | Superseded for accounts | The historical cleanup removed the old account code contract. On 2026-07-27, `accounts.code` was re-established as the explicit branch-scoped business code required by finance flows; machine and production-line decisions remain unchanged. |
| 2026-05-20 | Master import/export round-trip + SKU product codes | Passed | `/master-data/products` now has Import Excel UI and `/api/master-data/products/import` for all-or-nothing import from the exported `สินค้า` sheet. Product code validation now uses `SKU001-SKU99999`; production `products.code` was normalized to `SKU001...` after backup `maintenance.products_sku_code_backup_20260520135714` while preserving existing `products.id` values for transaction FKs. Customer phone is optional so exported customer rows with blank phone can import back. Supplier export now includes `รหัสผู้ดูแล`, and bank-account export fills blank transfer bank names as `ไม่ระบุ` so existing legacy rows can round-trip through import validation. Validation passed: `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `git diff --check`, and `npm run build --workspace @ns-scrap-erp/next`. |
...
| 2026-06-07 | Product receiving-status column removal | Passed + production DB applied | `/master-data/products` no longer exposes `ประเภทคลังที่จะรับเข้า` in filter/table/form/import/export. Prisma, product API/domain contracts, and transaction/report reads were updated to stop selecting `products.item_status`. Migration `supabase/migrations/20260607141612_drop_products_item_status.sql` drops `public.products.item_status` and `idx_products_status`; production was applied and migration history repaired. Verification returned `0` remaining `public.products.item_status` columns and `1` migration-history row for `20260607141612`. Validation passed: `npm run prisma:generate --workspace @ns-scrap-erp/next`, `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `npm run build --workspace @ns-scrap-erp/next`, and `git diff --check`. Local unauthenticated HTTP smoke returned the expected `/login?redirect=%2Fmaster-data%2Fproducts` redirect. |
...
- Final code strategy: keep explicit business codes only where they are meaningful business identifiers or externally referenced (for example branches, customers, suppliers, products, salespersons, and company accounts). Company account codes use `ACC<รหัสสาขา>-<ลำดับ 3 หลัก>` and are generated server-side; `account_no` remains the digit-only bank account number. Product business codes use `SKU001-SKU99999`; product internal ids are preserved separately for transaction FKs.
- Supplier duplicate rows by normalized name were cleaned in `production`, but supplier `code` still has duplicate values across different suppliers. Do not rewrite these codes until the final supplier code strategy is confirmed.
...
- Supplier master no longer stores or displays separate contact-person fields. The production migration backed up old supplier contact values to `maintenance.supplier_contact_backup_20260518` before dropping `contact`, `contact_title`, `contact_first_name`, and `contact_last_name` from `public.suppliers`.
...
- Production `director_employees` was reset on 2026-06-09 to 10 mock rows `P001`-`P010` for the redesigned company-person page. This is a development data reset only, not a production data migration.
...
- Expense category master no longer stores or displays a parent category. Existing production rows had no parent references before `parent_id` was dropped.
