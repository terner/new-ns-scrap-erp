...
| 2026-05-23 | Product receipt status + warehouse type routing | Passed app validation; DB apply pending | Reintroduced `products.item_status` as user-facing `รับเข้าเป็น` with RM/WIP/FG/SCRAP choices and re-enabled `/master-data/warehouses` as the stock-location master with required `warehouses.type`. Purchase bill STOCK create/edit now routes each line to an active warehouse in the selected branch whose type matches the product receipt status, falling back to legacy code/name hints only when type is absent. Migration file added: `supabase/migrations/20260523000100_add_warehouse_type.sql`; DB apply is still pending because dev-target migration history has a remote-only `20260519044843` entry and multiple local-only history rows that need repair/pull before a clean schema push. Validation passed: `npm run type-check --workspace @ns-scrap-erp/next`, `npm run lint --workspace @ns-scrap-erp/next`, `npm run build --workspace @ns-scrap-erp/next`, and `git diff --check`. |
...
- Product master uses `products.item_status` again as `รับเข้าเป็น` for stock receipt routing. `metal_group`, `grade`, `std_price`, and `std_cost` remain out of the active Next product form/export flow.
...
- Tax rate settings are now DB-backed master/config lists. Dev-target has `vat_settings` seeded with `VAT 7%` and `wht_settings` seeded with `WHT 3%`; `/admin/system-settings` exposes both active rates under the System module with percent-only editing. The shared master-data schema validates rate percent as `0-100`. `/purchase/bills` uses active VAT config for bill VAT calculation, and `/purchase/payments` uses active WHT config to compute read-only supplier-payment WHT in the API before save.
