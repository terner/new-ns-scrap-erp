...
- Supplier domestic/foreign classification is now part of the address section. Domestic suppliers use Thai postcode/province/district/subdistrict fields with `country_code = TH`; foreign suppliers use ISO country code plus international address line/city/state-region/postal fields and do not write Thai hierarchy columns. `ที่อยู่เต็ม/หมายเหตุที่อยู่` is retained as address metadata; the separate general `notes` field is no longer shown in the supplier modal or included in supplier export/import.
- Supplier tax ID input now follows the project validation rule: only digits can remain after typing/paste, capped at 13 digits for Thai party tax IDs, with the server schema still enforcing the saved value.
