...
- Admin Users & Permissions table mechanics for `/admin/users-permissions`, `/admin/users`, and `/admin/roles-permissions` now has a local static design checkpoint: the Users and Roles desktop tables use `ResizableTableHead`, persisted resizable widths, reset-width control, `colgroup`, fixed table layout, and client-side sorting after search; mobile cards use the same sorted row sets. Browser QA remains pending for these routes.
...
- Fixed assets finance pages: `/finance-accounting/asset-register`, `/finance-accounting/depreciation`, `/finance-accounting/asset-disposal`, and `/finance-accounting/asset-overview`
  ...
  - Follow-up correction for `/finance-accounting/depreciation`: both the Depreciation History table and Pending Assets table now use sortable business headers with inactive/active up/down indicators, apply client-side sorting before rendering/pagination, share sorted row sets with mobile cards, and use fixed table layout so resize handles visibly adjust columns like `/dual-costing/cost-pool`.
...
- `/finance-accounting/posting-rules`
  ...
  - GL posting, account mapping schema, readiness API, Accounting Period lock policy, rule versioning, report-helper behavior, and DB state were not changed.
  - Browser QA remains pending.
- `/tracking/customer`
...
### Runtime Table Contract Rescan - 2026-08-01

- Static AST/text inventory covered every direct non-test TSX table surface under `apps/next/src`: 201 native runtime JSX tables comprise 200 `ns-table` tables plus one explicit financial-hierarchy exception. Another 25 call sites use the shared `Table`, which adds `ns-table` centrally. Separately, 14 HTML-string document tables across 11 files keep their own print styles and are not runtime UI tables.
- The Product Tracking detail `รายการขาย` regression was corrected together with Customer/Supplier/Product detail helpers: dates/documents/statuses now use semantic non-wrapping columns, document values use mono text, numbers use right-aligned tabular numerals, and ambiguous `ขาย` in allocation detail is now `เอกสารขาย`.
- A full formatted-date cell scan found and corrected 26 additional date/time cells across 13 active files so date values cannot split across lines while intentional metadata/status remains on its second line.
- Newly merged FCD Conversion and FCD Revaluation lists were the only high-confidence primary runtime grids missing the full sort/resize contract; both now use `ResizableTableHead`, persisted resizable widths, `colgroup`, fixed layout, horizontal overflow, and width reset without changing finance handlers or API behavior.
- Shared CSS now keeps raw `text-left` / `text-right` header intent aligned with body cells and removes the conflicting dark per-header border override. Repository guards cover `ns-table`, date/time nowrap, resizable width, and row-action contracts.
- Stock Planning purchase/overview/detail/calendar tables now use the same semantic alignment contract, and the WTI/WTO primary list centers document/date values while keeping the party name left-aligned; focused source-contract tests lock these surfaces against regression.
- The independent follow-up found and closed variable-backed document/date gaps that formatter-only scanning could not prove: Migration Tools snapshot dates, Stock Transfer document/update values, Stock Planning shortage/detail/calendar document values, and Product Tracking sales headings now stay contextual and non-wrapping. The runtime guard keeps an explicit maintainable inventory for those direct-value cells instead of a 92-candidate generic regex with known false positives.
- Admin Users now uses the shared action menu directly and aligns descriptive columns left; Daily Petty Advance centers and preserves document/date/action values; FCD Conversion and Revaluation add sorted mobile cards, real loading/empty states, and the shared mobile action trigger while retaining their desktop sort/resize/fixed-layout behavior.
- Independent acceptance initially found that numeric body/footer cells relied on `text-right` without a complete shared no-wrap contract and that FCD Conversion/Revaluation status cells could still wrap. The shared `ns-table` rule now applies non-wrapping tabular numerals to every right-aligned numeric body/footer cell, both FCD status cells are non-wrapping on Desktop/Mobile, and the repository guard now scans direct numeric/status values instead of checking only headers and date/document cells.
- Final validation after that correction passed the focused table-contract suite at 15 files / 83 tests, workspace lint with 0 errors and 10 pre-existing/out-of-scope warnings, workspace type-check with an 8 GB Node heap, and the SIT-env webpack production build with 331 routes. The previous final production login/RSC/navigation guard passed 112 routes / 225 checks with 0 failures; the final diff check is recorded in the active handoff.
- Representative Codex Browser QA on the final production build passed Desktop routes `/daily/weight-ticket-list`, `/production/report`, `/stock/planning`, `/stock/balance`, `/finance/cash-position`, `/finance-accounting/working-capital`, `/purchase/po-buy`, `/sales/po-sell`, and `/master-data/customers`. Mobile verification passed Weight Ticket cards, Production Report, Stock Planning, and Customer cards; no page-level overflow or unwanted ellipsis was observed in the checked real-data states, and the sweep recorded zero console errors.
- `/tracking/product` rendered but had no current-period rows, so Product Detail `รายการซื้อ` / `รายการขาย` could not be visually reopened with real data. The corrected sales-table date/document geometry is therefore proven by final source inspection and focused contract tests rather than a fresh populated-state screenshot. The Codex Browser session later became unavailable and no alternate browser was used.
- The former `/dual-costing/cost-allocation-ledger` and `/tracking/customer` `P2022` error was resolved on 2026-08-01 by promoting existing migration `20260727110000_add_trading_allocation_fact_cost_pool_entry.sql` to the verified SIT target. Preflight found no existing allocation-fact rows, so no business data was changed or backfilled; postflight confirmed the columns, FK, indexes, and migration history, while authenticated smoke returned HTTP 200 across Customer/Product Tracking, Allocation Ledger, Trading Matching, and Trading Dashboard. No UI or runtime fallback was added because schema parity is the correct contract.
- Fresh independent acceptance after the numeric/status correction returned `ACCEPTED`; no runtime-table contract gap remains in the reviewed artifact.

### Single-line Form and Filter Height Contract Sweep - 2026-08-02

- What: normal business-entry `Input`, `Select`, search/combobox, date, money, and native single-line controls now use the 40px `h-10` baseline. Search/date/select controls inside list filters explicitly use the compact 36px `h-9` baseline instead of inheriting the form default.
