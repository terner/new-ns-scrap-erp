...
- Done on 2026-06-08: `/daily/payment-approval` status filter now follows the shared list-page segmented filter design from `docs/design.md`: `สถานะ:` label, `ทั้งหมด` reset segment first, slate active/inactive styling, and multi-select toggle behavior for `ยังไม่อนุมัติ`, `อนุมัติแล้ว`, and `ยกเลิกแล้ว`. This is a UI filter behavior/design alignment; the payment approval API contract is unchanged.
