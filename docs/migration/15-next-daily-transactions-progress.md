...
- WTI/WTO UI update on 2026-05-25: `/daily/weight-tickets` now uses the target menu name `ชั่งสินค้า / รับ-ส่งของ` and captures WTI/WTO document type, branch, supplier/customer, vehicle plate, product weights, deduction mode (`ไม่หัก`, `หัก`, `หัก%`), image names per product line with at least one image required per line, and remark in localStorage only. Document number, document date/time, and entered-by are generated on save and are not previewed in the form. `/daily/weight-ticket-list` now exists as the UI/localStorage list/search/detail surface for `รายการใบรับ-ส่งของ`.
- WTI/WTO vehicle-image follow-up on 2026-05-25: the form now places `รูปภาพรถส่งของ` beside `ทะเบียนรถ`, stores vehicle image names separately from product-line image names in the localStorage prototype, opens uploaded images in a preview dialog, and hides the deduction value input when the line deduction mode is `ไม่หัก`.
