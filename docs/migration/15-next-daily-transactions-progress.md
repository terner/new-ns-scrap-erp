...
- Done on 2026-06-07: `/daily/expense` `ผู้รับเงิน` now supports searching active Customer, Supplier, Sale, and employee names while still allowing manual free-text entry. Suggestions are loaded from the expense GET payload and saving continues to write the existing text value to `expenses.payee`.
