...
- Added `GET /api/daily/weight-ticket-form-drafts/team` and highlighted `กำลังกรอก` rows inside the existing WTI/WTO table and mobile card list; there is no separate draft tab. Clicking a row/card opens a read-only detail modal with type, drafter, branch, party, product/line summary, aggregate weights and latest update. The modal follows the shared neutral-header/grouped-card/sticky-footer detail-dialog pattern and reuses the list's existing projection state and three-second visible-page poll, so opening it creates no extra endpoint request or interval. A server-generated keyed-HMAC row token keeps the modal attached to the exact selected owner+scope snapshot without exposing either internal value. The bounded reader visibly warns when more than 100 candidate snapshots exist. Before a new draft selects a branch it is returned to every permitted viewer as `ยังไม่เลือกสาขา`, allowing another user to see that work has started; branch-scoped party and warehouse labels remain omitted until the server has a trusted branch scope. Party/product/warehouse display labels resolve server-side from active masters in that scope; an unresolved or out-of-scope reference is omitted and never falls back to snapshot/client text. It may identify the resolved selected product, resolved selected warehouse, or current gross weight, but never `vehicleNo`, client-entered labels, free-text notes/remarks, attachment filenames/URLs, IDs/scope keys, prior values, or an action/context/description supplied by the client.
...
- Both working-draft migrations are applied to dev-target: `20260721030145_create_weight_ticket_form_drafts.sql` and `20260721120000_add_weight_ticket_form_draft_visibility_branch.sql`. Postflight confirmed the nullable trusted branch FK, `(visibility_branch_id, updated_at desc)` index, migration-history row, RLS, and no `anon`/`authenticated` grant. Focused draft/API/filter/modal Vitest passes `45/45`, full ESLint has `0` errors with four pre-existing warnings, targeted ESLint and type-check with a 4 GB Node heap pass, `npm audit --audit-level=high --omit=dev` reports zero vulnerabilities, production build generates `325/325` routes, health returns HTTP 200, and `git diff --check` passes. Full Vitest has 16 failures only in unchanged Finance, Sales Plan, PO Sell, and shared resizable-table contract files. Production browser smoke confirmed draft writes and `/team` return 200 and the existing table shows one visible `กำลังกรอก` row within 3 seconds with zero console errors. Feature commit `f88799fc2` is pushed to `new-origin/peach`; promotion to `dev`, UAT, or `main` remains separate.
...
- Done on 2026-07-13: หลัง transaction สร้าง PMT จาก `/purchase/payments` สำเร็จ ระบบ enqueue/execute LINE source `purchase_payment` แบบ post-commit; `PMT` เลือกกลุ่มแยกได้ใน `/admin/line-settings`, ใช้ explicit fail-closed routing, รวมหลาย payment/PMA/account-split rows ใน Flex เดียว, mask เลขบัญชีเหลือท้าย 4 หลัก และ LINE failure ไม่ rollback การจ่ายเงินจริง.
- Done on 2026-07-13: หลัง RCP เปลี่ยนเป็น active จาก `/sales/receipts` สำเร็จ ระบบ enqueue/execute LINE source `customer_receipt` แบบ post-commit; `RCP` เลือกกลุ่มแยกได้, pending/cancel ไม่ส่ง, cancel-and-reissue ส่งเฉพาะเลขใหม่, Flex รวมหลาย SB/account rows โดยไม่อ่านเลขบัญชีเต็ม และ LINE failure ไม่ rollback การรับเงินจริง.
