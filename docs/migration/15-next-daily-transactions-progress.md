...
- Done on 2026-05-28: local production applied `20260528190000_restore_payment_approval_doc_no_unique.sql`. Duplicate PMA rows from the temporary reuse experiment were renumbered safely and `payment_approvals.doc_no` is unique again at the database level.
...
- Ops checkpoint on 2026-06-08: production transaction/document data was cleared after the PMA/PMT numbering fixes. The cleanup used a transaction-table whitelist only, checked FK cascade scope before execution, and verified all 51 cleared transaction tables returned `0` rows afterward while master data counts remained intact. This intentionally removes old test PMA suffix rows from dev data rather than adding runtime compatibility for them.
...
- DB migration apply is pending. `supabase migration list --db-url "$SUPABASE_DB_URL"` on 2026-05-23 showed remote-only migration `20260519044843` plus multiple local-only migration history gaps, so production needs migration history repair/pull before `20260523000100_add_warehouse_type.sql` can be pushed cleanly.
...
- Supplier payment save now refreshes the linked purchase bill AP fields in the same transaction: `paid_amount`, `payable_balance`, and `status` are recomputed from active `payments` rows by `bill_id`; overpayment is rejected and edits that move a payment to another bill refresh both old and new bills. Data dictionary updated at `docs/data-dictionary/purchase-bills.md`, and a production schema-only snapshot was dumped to `reports/db_audit/production_schema_20260522.sql`.
...
- Prisma already has target models for:
  ...
  - `receipts`
  - `bill_swap_history`
- Petty advance target tables added in `production` and migration file:
