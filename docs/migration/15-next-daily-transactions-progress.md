...
- Done on 2026-05-29: newly created `ADV` rows now enter `/daily/payment-approval` as pending supplier-payment items. The approval API/UI accepts `advance_payment` source rows, shows them in the AP tab with `ADV` source labeling, and marks `supplier_advance_payments.status='approved'` when a PMA snapshot is created.
- Done on 2026-05-29: cancelling a queued PMA item for `advance_payment` now reopens the source ADV by returning `supplier_advance_payments.status` to `pending_approval` when no other approved PMA remains for that ADV document.
...
- Done on 2026-05-30: split approval items now receive PMA item doc numbers in `PMA.../1`, `PMA.../2`, ... form. The `อนุมัติแล้ว` filter on `/daily/payment-approval` now shows these split rows as approval items with PMA item doc no + source doc reference instead of collapsing them back into one source-document row.
...
- Remaining correction block after 2026-05-30 decision:
  ...
  - Fix `/purchase/payments` so all valid approved PMA items appear in the waiting queue, including the current ADV-related visibility bug that surfaced during review.
  ...
  - Complete the payment-history cancellation path so voucher rollback updates history UI/status cleanly without approval-state rollback.
...
- Product master now exposes the stock receipt status (`products.item_status`) as `รับเข้าเป็น` with RM/WIP/FG/SCRAP choices. Purchase bill `STOCK` create/edit uses that value per line for stock status / `output_category`; it does not use product status to auto-pick a warehouse.
- Warehouse master now has `warehouses.type` with RM/WIP/FG/SCRAP choices as master metadata. Purchase bill stock routing no longer auto-picks by warehouse type, code, name, or legacy hints; users must choose an active warehouse from the selected branch, and the API validates same-branch warehouse before writing `purchase_bills.warehouse_id` and `stock_ledger.warehouse_id`.
