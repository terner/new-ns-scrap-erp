...
- รายการย่อยในกลุ่มต้นทุนใช้คำว่า `รายการ` เท่านั้น ไม่ใช้ `รายการต้นทุน`, `ล็อต`, `ลอท` หรือ `Lot` ในข้อความที่ผู้ใช้เห็น.
...
- Modal หรือ mobile bottom sheet ต้องมี visible dismiss action เพียงหนึ่งตำแหน่ง ห้ามแสดงทั้งปุ่ม `X` และปุ่ม `ปิด` / `ยกเลิก` / `ตกลง` ที่ทำเพียงปิด surface เดียวกันพร้อมกัน.
- Shared `DialogContent` ที่มีปุ่มออกจาก modal ใน header/footer ต้องกำหนด `hideClose`; ปุ่มเปลี่ยนสถานะธุรกิจ เช่น `ยืนยันยกเลิก`, `จบงาน`, หรือลบข้อมูล ไม่ถือเป็น dismiss action.
- Shared `MobileFilterSheet` ต้องใช้ `role="dialog"`, `aria-modal`, heading ที่ผูกด้วย `aria-labelledby`, ใช้ footer action เป็นทางออกที่มองเห็นได้, ปิดด้วย `Escape`, จำกัด `Tab` focus ไว้ใน sheet และคืน focus ให้ control ที่เปิด sheet เมื่อปิด.
- Shared `MobileFilterSheet` ใช้ action ใน footer เป็นเส้นทางออกที่มองเห็นได้เพียงชุดเดียว; backdrop click ยังใช้ dismiss ได้ แต่ไม่เพิ่มปุ่ม `X` ซ้ำใน header.
- Detail/create modal ที่ปุ่มมีผลเพียงปิดโดยไม่บันทึก ให้ใช้ wording เดียวคือ `ปิด`; หน้าใดต้องใช้คำอื่นเพราะมี discard/reset behavior ต้องบันทึกเหตุผลใน flow document.
...
- search field:
  ...
  - active-field baseline applies to every editable field: input, select, textarea, date field, search field, combobox, and input group. Keep each theme's existing field background and idle border. While active, use one blue focus border `#3B82F6` with a `3px` translucent blue ring in both themes. Red remains reserved for validation/error state.
  - every field that the user types into or selects—including business-entry fields, search/report filters, and date pickers—uses the persistent pale-yellow background `#FFF7CC` in Light mode and the shared translucent yellow token in Dark mode, whether the field is required or optional. The yellow remains after a valid value is entered so office users can distinguish data they own from reference data. Automatically populated values, calculated values, genuinely read-only fields, disabled fields, checkboxes, radios, and native file controls stay on their normal neutral surface. Validation/error state overrides yellow with red, while keyboard/mouse focus keeps the shared blue border/ring without changing the yellow surface to white.
  - business-entry intent should use a semantic entry scope: native write/auth `form`, shared `DialogContent`, or an explicit `data-ns-field-scope="entry"` on form-free business surfaces. Filter sheets and any filter surface nested inside an entry context should use `data-ns-field-scope="filter"` so their editable/search/date controls receive the same yellow contract; the shared CSS also has a global editable-control fallback so an older active page cannot silently lose the yellow treatment while it is being migrated. Do not infer intent from `type="search"`, field size, placeholder, or whether a label is hidden. A visible red `*` still does not replace native `required` / `aria-required="true"` accessibility and validation semantics. Dependent controls stay neutral while disabled. A combobox may use an internal `readOnly` text input to force menu selection; when the control remains user-editable, mark that implementation-only input with `data-manual-entry-readonly="true"` so it is not mistaken for business read-only data.
- date range controls:
  - height target: `h-9`
...
- mobile card action trigger: use the same `TableActionButton` and compact dropdown, but show one full-width outlined button labelled `จัดการ` instead of the ellipsis. Group every row-level command inside that menu; do not render a row of separate `พิมพ์` / `แชร์` / `แก้ไข` / `ยกเลิก` buttons on the card. The labelled trigger uses the shared `mobileLabel` mode (`h-9 w-full`, white surface, slate border, compact `14px` text) and spans the card's inner content width; its dropdown reads the Radix trigger-width variable so the opened menu spans the same width on mobile while the Desktop ellipsis retains the compact minimum menu width. This gives mobile users one discoverable touch target and a visually connected menu without squeezing several narrow controls into limited card width.
- action dropdown event boundary: because the compact menu is rendered through a portal, its click event must stop at the shared menu content. Selecting `แชร์`, `พิมพ์`, `แก้ไข`, or another command must never bubble into the parent row/card and open its detail view as a second action.
- canonical `จัดการ` column width is `defaultWidth: 72`, `minWidth: 64`, and `maxWidth: 88`; the max width intentionally clamps older saved wide values after action buttons move into the menu. Use the visible header label `จัดการ` for management columns; labels such as `เปิด` or `เลือก` remain separate workflow columns and do not use this contract.
...
- ห้ามวาง KPI/stat cards ซ้อนอยู่ใน outer card/panel อีกชั้นบน dashboard/report surfaces; ถ้าต้องมีหัวข้อ ให้ใช้ heading + grid เปล่า หรือใช้ stat columns/เส้นแบ่งภายใน panel แทน card-in-card
...
- Optional KPI delta must use the shared `delta` prop only: small up/down/flat icon + compact percent/number next to the main value. Delta values must come from real comparable data (for example previous equivalent period), not mock values; color meaning follows the KPI business meaning, not raw direction alone (for example Expenses/AP up = bad, Revenue/Cash/Profit up = good). This is a metric-card delta badge, not a sparkline/chart.
...
การ์ดพนักงานขายใน `/sales-commission` overview ใช้ layout ที่ผู้ใช้อนุมัติจากภาพอ้างอิงวันที่ 2026-07-10: ภายในการ์ดเป็น grid 2 คอลัมน์ โดย `บิล` / `Supplier` อยู่แถวแรก, `น้ำหนักรับซื้อ` / `น้ำหนักที่ได้คอม` อยู่แถวสอง, `ยอดรับซื้อรวม` / `ยอดซื้อที่ได้คอม` อยู่แถวสาม และ `ค่าคอมเดือนนี้` กว้างเต็ม 2 คอลัมน์ด้านล่าง. ใช้สีข้อความตามความหมายโดยไม่ใส่ icon และห้ามแทน layout นี้ด้วย generic `KpiCard` แบบเรียงแนวตั้ง เว้นแต่ผู้ใช้อนุมัติการออกแบบใหม่.
...
- **Pagination & Table**: ใช้ Resizable Columns (Opt-in) ตารางมีเส้นคั่นแถวแนวนอน `1px` สี `--color-scrap-line` และ row action อยู่คอลัมน์ขวาสุด
...
- **ตัวกรองแบบสไลด์ขึ้นจากด้านล่าง (Filter Bottom Sheet)**:
  ...
  - backdrop ใช้ dark scrim ที่คงค่า opacity เดิมทั้ง Light/Dark; ห้ามอ้าง semantic slate token ที่ถูก remap ตาม theme จนกลายเป็นพื้นขาวโปร่งใน Dark Mode
  - หัวข้อระบุ "ตัวกรองรายการ..." โดยไม่เพิ่มปุ่มปิดกากบาทที่ทำงานซ้ำกับ visible footer action; backdrop/Escape ยังคงปิด sheet และคืน focus ตาม shared dialog behavior
  - ภายในเป็นช่องกรอกวันที่ (จัดวางเคียงคู่กัน 2 คอลัมน์) และปุ่ม Segmented Filter ขนาดเล็กสำหรับสถานะต่างๆ
  - ด้านล่างสุดมีปุ่ม **`ล้างตัวกรอง` (outline)** และ **`ใช้ตัวกรอง` (สีน้ำเงิน)** วางเคียงคู่กันแบ่งครึ่งพื้นที่สมมาตร; การแก้ค่าใน sheet ต้องอยู่ใน draft และยังไม่เปลี่ยน list query จนกด `ใช้ตัวกรอง` ส่วนการปิดด้วย backdrop/Escape ต้องทิ้ง draft
...
## Customer-Approved Canonical Full-Page Reference

As of 2026-07-12, `/daily/weight-ticket-list` is the explicit customer-approved canonical full-page UX/UI reference for the active Next app. New pages and design-normalization work must start from the current rendered state of this route, not from an inferred or page-local style.

Use this page as the exact visual and composition baseline for:

- page hierarchy: app shell, title/breadcrumb, line tabs, filter surface, count/pagination row, and primary data surface
- control placement and density: search/date/select/clear controls, segmented status filters, page actions, row actions, and responsive overflow behavior
- color system in both Light and Dark modes: neutral surfaces/borders/text plus blue primary, green export/print, slate secondary, status colors, and red destructive actions
- modal/dialog/mobile-sheet presentation used by this flow: shell, backdrop, header/body/action grouping, spacing, color hierarchy, and responsive behavior
- table presentation: shell, header treatment, column alignment by data meaning, resizable columns, row density, status cues, action spacing, and mobile-card fallback
- pagination presentation: total count, page-size selector, previous/next controls, and `หน้า X / Y` placement

“Exact baseline” means preserving the same component hierarchy, spacing scale, color roles, control sizing, alignment logic, and Light/Dark geometry. Copy only the visual/interaction system—not WTI/WTO-specific fields, statuses, labels, permissions, or business actions. A page may differ where its business flow requires different information or controls, but any visual/layout deviation must be explicitly approved and recorded in `docs/migration/00-current-work.md`.

The `/production/orders` screenshot remains supporting evidence for the same `p-2` header / `p-3` body cell-density values; it is not a competing full-page baseline.
...
- row height: balanced, compact, and readable; use the `/production/orders` density reference below so spacing stays identical across pages
- primary list/report table reference is `/daily/weight-ticket-list`: desktop table container should use `hidden md:block overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm`, table should use `min-w-full text-sm`, header should use `bg-slate-100` without default vertical header separators, body rows should use the shared thin semantic horizontal divider, body cells should use `px-3 py-3`, and numeric cells should use `tabular-nums`.
...
### Canonical Table Spacing / Density

`/daily/weight-ticket-list` remains the complete table-pattern reference for shell, resize, sorting, pagination, and mobile cards. For the exact horizontal/vertical breathing room inside desktop table cells, use `/production/orders` as the canonical density reference:

![Canonical table spacing from Production Orders](mockups/table-density-production-orders-canonical.png)

- approved visual source: `/production/orders` dark-mode screenshot captured on 2026-07-11; light and dark modes must use the same geometry
- header cells: `p-2`; resizable/sortable header content: `p-2 pr-4`; right-aligned numeric headers use `p-2 pr-3` and place the sort icon before the label so the label edge aligns with `p-3` body values
- body cells: `p-3` (equivalent to `px-3 py-3`), including numeric, status, and action columns
- implementation guard: `apps/next/src/app/globals.css` enforces `0.5rem` on normal `ns-table` headers and `0.75rem` on every `ns-table` body cell with `!important`; every runtime JSX table must therefore keep the `ns-table` class instead of defining page-local density
- two-line cells: primary value plus secondary `text-xs` metadata separated by `mt-0.5`; let content determine row height
- body rows: use the shared horizontal `1px` `--color-scrap-line` divider together with stable whitespace and hover treatment; do not add vertical column borders
- scope: every active runtime data table, including primary lists, tab tables, drilldowns, and nested/secondary tables. Print/PDF/HTML-string document tables remain governed by their document stylesheet. Any spacing exception requires explicit user approval and a note in `docs/migration/00-current-work.md`.
...
- ต้องปรับ `defaultWidth` จาก populated render ที่ viewport desktop มาตรฐาน ไม่ใช่ดูเฉพาะว่าข้อความไม่ล้น: ระยะซ้าย-ขวาของข้อมูลแต่ละคอลัมน์ต้องสมดุล, คอลัมน์สั้นอย่างสาขา/ทะเบียน/สถานะห้ามมีช่องว่างเกินเหตุ, และ action column ต้องกว้างพอดีกับ valid action set โดยไม่ผลักข้อมูลหลักออกไปไกลเกินจำเป็น; ยอมให้มี internal horizontal scroll ระดับพอประมาณแทนการบีบข้อความหรือปล่อยช่องว่างไม่สม่ำเสมอ
- `<colgroup>` ต้องใช้ `getColumnStyle()` กับทุก resizable column รวมคอลัมน์สุดท้าย ห้ามตัด `width` ของคอลัมน์สุดท้ายออกเพื่อให้มันรับพื้นที่ว่างเพียงคอลัมน์เดียว; เมื่อ `table.ns-table` เต็ม container browser ต้องกระจายพื้นที่ส่วนเกินตามความกว้างที่กำหนดของทุกคอลัมน์ โดยให้คอลัมน์ข้อความยาวมี `defaultWidth` มากกว่าคอลัมน์สถานะ/ตัวเลข/จัดการตามเนื้อหาจริง
...
### Table / Subtle Row Dividers

ใช้กับ active runtime `table.ns-table` ทุกหน้า เพื่อช่วยให้ผู้ใช้ไล่อ่านข้อมูลตามแถวได้ชัดโดยไม่ทำให้ตารางดูหนัก:

- ใช้เส้นแนวนอน `1px` สี `--color-scrap-line` (`#d9e2ec` ใน Light mode) ระหว่างแถวใน `tbody`; แถวแรกไม่ต้องมีเส้นด้านบน ห้ามใช้ `slate-100` เพราะจางจนแทบมองไม่เห็นบนพื้นขาว
- Dark mode ใช้ค่า `--color-scrap-line` ที่ theme map เป็น `#334155` เพื่อให้เส้นยังเห็นได้แต่ไม่สว่างรบกวนสายตา
- ห้ามเพิ่มเส้นตั้งระหว่างคอลัมน์ และห้ามใช้เส้นเข้มซ้อนกับ row divider กลาง
- คง hover state และพื้นหลังสถานะ/ยกเลิกของแต่ละแถวไว้ตามเดิม
- ใช้ global selector ใน `globals.css` เป็น source of truth ไม่ทำ page-local `divide-y` variant เว้นแต่เป็น nested document table ที่มี requirement เฉพาะ
...
- ทุกคอลัมน์ข้อมูลถัดไปชิดขวาทั้งหัวตารางและข้อมูล รวมถึงเลขที่เอกสาร สถานะ badge และ `จัดการ` เว้นแต่เป็นคอลัมน์ข้อความเชิงบุคคล/คู่ค้า/วันที่ที่มี accepted surface ระบุไว้ด้านล่าง
- accepted surface จากภาพยืนยันวันที่ 2026-07-19 ให้ชิดซ้ายทั้งหัวและข้อมูลเฉพาะคอลัมน์ต่อไปนี้: `/purchase/bills` = `ผู้ขาย`, `/sales/bills` = `ลูกค้า`, `/purchase/receipt-vouchers` = `ผู้รับเงิน`, WTO ใน `/daily/weight-ticket-dashboard` = `คู่ค้า`, `/daily/weight-ticket-list` = `วันที่สร้าง` และ `ผู้ขาย`, `/sales/po-sell` = `ลูกค้า`, `/purchase/advance-payments` = `ผู้ขาย`
- ข้อยกเว้นนี้เป็น semantic alignment เฉพาะ surface ที่ระบุ ไม่ใช่คำสั่งให้เปลี่ยน global CSS หรือพลิกคอลัมน์สถานะ/จัดการ; คอลัมน์ตัวเลขและจำนวนเงินยังชิดขวาเสมอ
...
### Shared Base & Structure (Neutral Table Header Style)
...
- **Header**: แยกสีออกจาก Sidebar และใช้ชุดสีเดียวกับหัวตารางเสมอ: `border-b border-slate-200 bg-slate-100 text-slate-700 shrink-0`; ใน Dark mode ให้ใช้ semantic table-header tokens เดียวกับตาราง มีกฎเฉพาะสำหรับการแสดงรายละเอียดเอกสาร (Detail Modal):
  - Modal ที่ยังเป็น custom/manual overlay และยังย้ายมาใช้ shared `DialogHeader` ไม่ได้ ต้องติด `data-ns-dialog-header` บน wrapper ของหัวจริง เพื่อให้ global presentation contract บังคับ palette เดียวกัน ห้ามสร้าง page-local modal-header palette แยกเอง
  - **หัวข้อหลัก (Title)**: ให้ระบุรหัสเอกสารโดยตรงในหัวข้อหลักเสมอ เช่น `รายละเอียด {row.docNo}` (ใช้ `<DialogTitle>` ซึ่งกำหนด `text-slate-800` จาก shared component)
  - **คำอธิบายย่อย (Subtitle)**: ให้ระบุชื่อคู่ค้าหลัก (ชื่อลูกค้าหรือผู้ขาย) เป็นคำอธิบายใต้หัวข้อหลักเสมอ เช่น `{row.customerName}` หรือ `{row.supplierName}` (ใช้ `<DialogDescription>` ซึ่งกำหนด `text-slate-600` จาก shared component)
...
## Table Balanced-Width Layout Rules

เพื่อป้องกันทั้งตารางแหว่งไม่เต็มจอและช่องว่างก้อนใหญ่ในคอลัมน์สุดท้ายบนจอ Widescreen:

- **ทุกคอลัมน์ต้องมี Width**: ใน `<colgroup>` ของตาราง resizable ให้ทุก `<col>` ใช้ความกว้างจาก `getColumnStyle()` รวมถึงคอลัมน์สุดท้าย ห้ามเหลือเฉพาะ `minWidth` หรือ `undefined` ให้คอลัมน์สุดท้าย
- **Table Width**: ตัวตาราง (`<table>`) ต้องเต็ม container เสมอ; `table.ns-table` มี `width: 100%` และ `min-width: 100%` เป็นกฎกลาง ส่วน `tableMinWidth` ยังรักษา horizontal scroll เมื่อผลรวมคอลัมน์กว้างกว่าจอ
- **กระจายพื้นที่อย่างสมดุล**: พื้นที่ที่เหลือบนจอกว้างต้องกระจายตามความกว้างของทุกคอลัมน์ ไม่กองอยู่ใน `สถานะ` หรือ `จัดการ`; คอลัมน์ข้อความยาว เช่น ลูกค้า/ผู้ขาย/สินค้า ใช้ `defaultWidth` ที่เหมาะสม ส่วน action column กว้างพอดีกับ valid action set
...
- `apps/next/src/components/production/ProductionOrdersPageClient.tsx` (canonical table spacing/density reference: header `p-2`, body `p-3`, two-line metadata `mt-0.5`)
...
- `docs/mockups/table-density-production-orders-canonical.png` (approved visual example for table cell spacing)
...
- 2026-07-19: Added the accepted semantic alignment exception for the seven #161 table surfaces: the specified party/name/date text columns are left-aligned with matching bodies, numeric columns remain right-aligned, and status/action/global CSS are unchanged.

- 2026-07-18: Added the customer-approved office/ERP Required Field Highlighting convention across shared inputs, selects, searchable comboboxes, branch selectors, date/input groups, and active page-local field wrappers. Required fields owned by the user remain pale yellow before and after entry; optional fields remain neutral; automatic/calculated/read-only/disabled fields are excluded; red validation and blue focus retain precedence.

- 2026-07-18: Standardized the selected `มี VAT` control in both purchase and sales bill forms from amber to the shared blue selection treatment (`border-blue-500 bg-blue-50`); warning and validation surfaces keep their semantic colors.

- 2026-07-18: Standardized every editable control and composite input group on one customer-approved blue focus treatment: border `#3B82F6` with a `3px` translucent ring in Light and Dark modes. Red remains reserved for validation/error state.

- 2026-07-17: Separated shared Modal/Dialog headers from the dark Sidebar. `DialogHeader`, `DialogTitle`, and `DialogDescription` now use the same neutral slate palette and bottom divider as runtime table headers (`slate-100` / `slate-700` / `slate-200`), while Dark mode follows the shared semantic table-header tokens. Page-local legacy dark header classes must not override this shared palette.

- 2026-07-14: Added customer-approved subtle horizontal row dividers to every active `table.ns-table`. The first `slate-100` attempt was too faint in the rendered Light table, so the accepted rule uses the semantic `--color-scrap-line` token (`#d9e2ec` Light / `#334155` Dark); vertical column lines remain disallowed.
...
- 2026-07-11: Set `/production/orders` as the canonical spacing/density reference for every active runtime table. Headers use `p-2`, body cells use `p-3`, two-line metadata uses `text-xs mt-0.5`, row height remains content-driven, and the approved screenshot is stored at `docs/mockups/table-density-production-orders-canonical.png`. `/daily/weight-ticket-list` remains the full shell/resize/sort/pagination/mobile reference.

- 2026-07-10: Added Owner Daily control-page baseline from `/owner-daily`: use a compact date filter row, one cash-plan panel for available/expected-in/expected-out/gap, distinct actual-activity KPI cards, and shared line tabs for AR/AP/loan/expense/pending detail surfaces. Do not use oversized success/warning hero banners, duplicate cash-plan KPI cards, card-in-card KPI sections, decorative emoji labels, side-by-side squeezed AR/AP tables, or English table headers such as `Due` on Thai-first owner-control screens.
