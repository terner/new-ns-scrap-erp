...
- 2026-07-20: Superseded the required-only yellow-field rule with the customer-confirmed manual business-entry contract. Required and optional fields that users type into or select stay pale yellow; filters, automatic/calculated/read-only/disabled values, checkboxes, radios, and native file controls stay neutral; red validation and blue focus retain precedence. Entry/filter intent is now explicit through native forms, shared dialogs, and `data-ns-field-scope` markers instead of being inferred from control type.
