...
- **Footer**: ใช้ `DialogFooter` หรือ Sticky container ติดอยู่ด้านล่างสุดโดยตรึงไว้เพื่อไม่ให้เลื่อนหลุดสายตา โดยมีกฎเกณฑ์ดังนี้:
  - **ปุ่มหลัก (Save Button)**: ต้องใช้สไตล์สีน้ำเงินของระบบเสมอ (`bg-blue-600 hover:bg-blue-700 text-white font-normal`) เพื่อความเป็นอันหนึ่งอันเดียวกัน ไม่ว่าจะบันทึกเอกสารประเภทใด และให้ใช้ข้อความสั้นๆ ว่า **"บันทึก"** เท่านั้น
  - **ปุ่มยกเลิก/ปิด (Cancel Button)**: ให้ใช้สไตล์ `variant="outline"` และข้อความ "ยกเลิก" หรือ "ปิด"
  - **การจัดวางข้อมูลสรุปใน Footer บนหน้าจอเล็ก**: หากมีส่วนสรุปตัวเลข (Metrics) ในแถบ Footer เมื่อแสดงผลบนหน้าจอแคบ/แนวตั้ง (เช่น โหมดมือถือ หรือกล่องบีบตัว) ให้จัดวางข้อมูลสรุปนั้นอยู่กึ่งกลางหน้าจอ (Center alignment) เสมอ เพื่อความสมดุล
...
- **ใช้ Grouped Cards**: ให้รวบรวมฟิลด์ข้อมูลที่เกี่ยวข้องมาจัดกลุ่มอยู่ในการ์ดเดียวกัน เช่น "ข้อมูลหลัก" หรือ "ยอดเงินและจำนวน"
- **Card Styling**: แต่ละกลุ่มข้อมูลใช้การ์ดขอบโค้งมนมีเงาบางๆ: `rounded-lg border border-slate-200 bg-white p-5 shadow-sm`
...
- 2026-06-13: เพิ่มดีไซน์ไกด์ไลน์สำหรับ Modal/Detail Popup (AcexPOS Dark Header Style, Grouped Cards Layout, Mobile Responsive layout และการสลับตารางเป็นการ์ดบนมือถือ), ตำแหน่งปุ่มสร้าง/จัดการ (Desktop page action buttons) ใน Filter Row และ Table Auto-Stretch Layout Rules
