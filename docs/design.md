...
- search field:
  ...
  - required manual-entry fields use persistent pale-yellow background `#FFF7CC` in Light mode and the shared translucent yellow token in Dark mode. The yellow remains after a valid value is entered so office users can distinguish fields they own from reference data. Optional fields stay on the normal white/surface background. Automatically populated, calculated, read-only, and disabled fields must not use the yellow treatment. Validation/error state overrides yellow with red, while keyboard/mouse focus keeps the shared blue border/ring.
  - a visible red `*` is not sufficient by itself: the editable control must also expose native `required`, `aria-required="true"`, or live inside `data-manual-required="true"`. Otherwise the shared highlight and accessibility contract cannot identify it. Dependent controls stay neutral while disabled and receive the required treatment only after they become editable. A combobox may use an internal `readOnly` text input to force selection through its menu; when the menu remains user-editable, mark that implementation-only input with `data-manual-entry-readonly="true"` so it is not mistaken for business read-only data.
- date range controls:
  - height target: `h-9`
