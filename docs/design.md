...
4. **Pagination / Table Toolbar** ของ tab ที่กำลังแสดง
...
สำหรับ dashboard overview ที่มี global filters จำนวนมาก ให้ desktop แสดงตัวกรองหลักใน card เดียวก่อน hero เพื่อให้เห็น scope ชัดเจน ส่วน mobile ต้องไม่วาง form filter ยาวเต็มหน้า ให้ใช้แถบ compact สำหรับช่วงเวลา/summary และปุ่ม `ตัวกรอง` ที่เปิด `MobileFilterSheet` แทน. Filter card ควรเป็นพื้นขาว/neutral ไม่ใช้แถบดำหนักถ้า hero ด้านล่างเป็น gradient อยู่แล้ว; field ที่เป็น input/select/search ต้องอ่านค่าได้จริงและไม่ดูเหมือนช่องว่าง. ถ้า hero ใช้ gradient/color เป็น visual anchor แล้ว KPI cards ใต้ hero ควรเป็น white cards พร้อม accent สีเล็กๆ ไม่ใช้ full-gradient ทุกใบจนแย่งสายตากับ hero และไม่ใช้ dot/marker ที่ดูเหมือนสถานะ error/success โดยไม่มีความหมาย. Metric section ท้ายหน้าต้องลดค่าที่ซ้ำกับ KPI/hero และเก็บไว้เฉพาะ detail ที่ช่วย drill-down.

---

### 2. รูปแบบสำหรับหน้าจอคอมพิวเตอร์ (Desktop Layout)

- **Filter Row**: วางเป็นบรรทัดเดียวกัน ประกอบด้วย ช่องค้นหาหลัก (`min-w-[260px]`), Date Picker (จาก -> ถึง), และปุ่มล้างตัวกรอง (แสดงเมื่อมีการเลือกตัวกรอง)
- **Desktop Page Action Buttons**: ปุ่มแอ็กชันหลัก เช่น **`ส่งออก Excel` (ปุ่มสีเขียว)** และ **`+ [ชื่อรายการ] ใหม่` (ปุ่มสีเข้ม slate-900)** ให้จัดวางไว้ขวาสุดในแถวตัวกรองเดียวกัน โดยใช้คลาส `ml-auto` เพื่อความกระชับและไม่เพิ่มความสูงของแถวโดยไม่จำเป็น
...
- **ตารางข้อมูลแบบการ์ดย่อย (Dense Card-based List)**:
  ...
  - การ์ดแต่ละใบมีขอบมนและเงานุ่มนวล: `rounded-md border border-slate-200 bg-white p-4 shadow-sm hover:bg-slate-50`
