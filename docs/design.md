...
- search field:
  ...
  - business-entry intent must come from a semantic entry scope: native write/auth `form`, shared `DialogContent`, or an explicit `data-ns-field-scope="entry"` on form-free business surfaces. Filter sheets and any filter surface nested inside an entry context must use `data-ns-field-scope="filter"` so their editable/search/date controls receive the same yellow contract; do not infer intent from `type="search"`, field size, placeholder, or whether a label is hidden. A visible red `*` still does not replace native `required` / `aria-required="true"` accessibility and validation semantics. Dependent controls stay neutral while disabled. A combobox may use an internal `readOnly` text input to force menu selection; when the control remains user-editable, mark that implementation-only input with `data-manual-entry-readonly="true"` so it is not mistaken for business read-only data.
- date range controls:
  - height target: `h-9`
...
- 2026-07-20: Extended the customer-confirmed yellow-field contract to every editable/searchable/date control. Required and optional fields that users type into or select—including filters—stay pale yellow; automatic/calculated/read-only/disabled values, checkboxes, radios, and native file controls stay neutral; red validation and blue focus retain precedence. Entry/filter intent is explicit through native forms, shared dialogs, and `data-ns-field-scope` markers instead of being inferred from control type.
