...
begin
  if exists (
    ...
    insert into public.payment_methods (id, name, active)
    values
      ('PM-001', 'เงินสด (Cash)', true),
      ('PM-002', 'เงินโอน (Bank Transfer)', true),
      ('PM-003', 'เช็ค (Cheque)', true),
      ('PM-004', 'พร้อมเพย์ (PromptPay)', true),
      ('PM-005', 'โอนเงินต่างประเทศ (International Transfer)', true),
      ('PM-006', 'โอนเงิน FCD (FCD Transfer)', true)
    on conflict (id) do update
    set
      name = excluded.name,
      active = excluded.active,
