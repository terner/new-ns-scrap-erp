...
active_approvals as (
  select
    ...
    sum(coalesce(apt.settled_amount, 0)) as settled_amount,
...
computed_status as (
  select
    ...
    case
      ...
      when coalesce(aa.settled_amount, 0) >= sap.amount - 0.01 then 'paid'
      when coalesce(aa.settled_amount, 0) > 0.01 then 'partially_paid'
